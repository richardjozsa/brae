...
__global__ void laplKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                           const scalar* __restrict__ gamma, const scalar* __restrict__ dc,
                           const scalar* __restrict__ w, const scalar* __restrict__ magSf,
                           scalar* __restrict__ ifCoeff, scalar* __restrict__ diag, int addToDiag) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
...
__global__ void convKernel(int n, const label* __restrict__ own, const scalar* __restrict__ phi,
                           scalar* __restrict__ ifCoeff, scalar* __restrict__ diag) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const scalar p = phi[j];
    ifCoeff[j] += (p < 0.0) ? p : 0.0;                  // off-diag += min(phi,0)
    atomicAdd(&diag[own[j]], (p > 0.0) ? p : 0.0);      // diag    += max(phi,0)
}
__global__ void momKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                          const scalar* __restrict__ nu, const scalar* __restrict__ dc, const scalar* __restrict__ w,
                          const scalar* __restrict__ magSf, const scalar* __restrict__ phi,
                          scalar* __restrict__ ifCoeff, scalar* __restrict__ diag) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
...
__global__ void addHKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                           const scalar* __restrict__ ifCoeff, const scalar* __restrict__ psi,
                           const scalar* __restrict__ V, scalar* __restrict__ H) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    atomicAdd(&H[own[j]], -ifCoeff[j] * psi[nbr[j]] / V[own[j]]);
}
__global__ void offSumKernel(int n, const label* __restrict__ own, const scalar* __restrict__ ifCoeff,
                             scalar* __restrict__ sumOff) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    atomicAdd(&sumOff[own[j]], fabs(ifCoeff[j]));
}
__global__ void fluxKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                           const scalar* __restrict__ w, const scalar* __restrict__ Hx,
                           const scalar* __restrict__ Hy, const scalar* __restrict__ Hz,
                           const scalar* __restrict__ Sfx, const scalar* __restrict__ Sfy,
                           const scalar* __restrict__ Sfz, scalar* __restrict__ phi) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const label o = own[j], nb = nbr[j]; const scalar wj = w[j], wn = 1.0 - wj;
    const scalar fx = wj * Hx[o] + wn * Hx[nb], fy = wj * Hy[o] + wn * Hy[nb], fz = wj * Hz[o] + wn * Hz[nb];
    phi[j] = fx * Sfx[j] + fy * Sfy[j] + fz * Sfz[j];
}
__global__ void divAddKernel(int n, const label* __restrict__ own, const scalar* __restrict__ phi,
...
__global__ void fluxCorrKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                               const scalar* __restrict__ ifCoeff, const scalar* __restrict__ p,
                               scalar* __restrict__ phi) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    phi[j] -= ifCoeff[j] * (p[nbr[j]] - p[own[j]]);     // snGrad(p) flux: -coeff*(p_nbr - p_own)
}
__global__ void cycTensorDivKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
    ...
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const int o = own[j], nb = nbr[j]; const scalar wf = w[j], sx = Sfx[j], sy = Sfy[j], sz = Sfz[j];
    ...
    scalar sn[9]; for (int q = 0; q < 9; ++q) sn[q] = sigmaC[q*nC + nb];
    if (rotational) {
        scalar R[9]; for (int q = 0; q < 9; ++q) R[q] = fT[q*n + j];
        scalar M[9];                                       // M = R*sigma
        for (int i = 0; i < 3; ++i) for (int l = 0; l < 3; ++l) {
            scalar s = 0; for (int k = 0; k < 3; ++k) s += R[3*i+k]*sn[3*k+l]; M[3*i+l] = s; }
        scalar sr[9];                                      // sigma' = M*R^T : sr[i][jj] = sum_l M[i][l]*R[jj][l]
        for (int i = 0; i < 3; ++i) for (int jj = 0; jj < 3; ++jj) {
            scalar s = 0; for (int l = 0; l < 3; ++l) s += M[3*i+l]*R[3*jj+l]; sr[3*i+jj] = s; }
        for (int q = 0; q < 9; ++q) sn[q] = sr[q];
    }
    scalar d[3];
    ...
    atomicAdd(&dX[o], d[0]); atomicAdd(&dY[o], d[1]); atomicAdd(&dZ[o], d[2]);   // raw sum (= V*fvc::div), like tensorDivKernel
}
__global__ void gradAddKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                              const scalar* __restrict__ w, const scalar* __restrict__ psi,
                              const scalar* __restrict__ Sfx, const scalar* __restrict__ Sfy,
                              const scalar* __restrict__ Sfz, const scalar* __restrict__ V,
                              scalar* __restrict__ gx, scalar* __restrict__ gy, scalar* __restrict__ gz) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const label o = own[j]; const scalar fv = (w[j] * psi[o] + (1.0 - w[j]) * psi[nbr[j]]) / V[o];
    atomicAdd(&gx[o], Sfx[j] * fv); atomicAdd(&gy[o], Sfy[j] * fv); atomicAdd(&gz[o], Sfz[j] * fv);
}
// ---- rotational helpers ---- forwardT packed (3*i+j)*n + face ; (forwardT·v)[i] = sum_k fT[(3i+k)n+j]*v_k
__device__ __forceinline__ void rotNbr(const scalar* fT, int n, int j, scalar vx, scalar vy, scalar vz,
...
__global__ void scaleImplicitKernel(int n, const scalar* __restrict__ ifc, const scalar* __restrict__ fT,
...
__global__ void fluxRotKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                              const scalar* __restrict__ w, const scalar* __restrict__ Hx,
                              const scalar* __restrict__ Hy, const scalar* __restrict__ Hz,
                              const scalar* __restrict__ fT, const scalar* __restrict__ Sfx,
                              const scalar* __restrict__ Sfy, const scalar* __restrict__ Sfz, scalar* __restrict__ phi) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const label o = own[j], nb = nbr[j]; const scalar wj = w[j], wn = 1.0 - wj;
    scalar rx, ry, rz; rotNbr(fT, n, j, Hx[nb], Hy[nb], Hz[nb], rx, ry, rz);
    const scalar fx = wj*Hx[o] + wn*rx, fy = wj*Hy[o] + wn*ry, fz = wj*Hz[o] + wn*rz;
    phi[j] = fx*Sfx[j] + fy*Sfy[j] + fz*Sfz[j];
}
__global__ void addHRotKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                              const scalar* __restrict__ ifc, const scalar* __restrict__ fT,
                              const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz,
                              const scalar* __restrict__ V, scalar* __restrict__ Hx, scalar* __restrict__ Hy, scalar* __restrict__ Hz) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const label o = own[j], nb = nbr[j];
    scalar rx, ry, rz; rotNbr(fT, n, j, Ux[nb], Uy[nb], Uz[nb], rx, ry, rz);
    const scalar c = ifc[j] / V[o];
    atomicAdd(&Hx[o], -c*rx); atomicAdd(&Hy[o], -c*ry); atomicAdd(&Hz[o], -c*rz);
}
__global__ void gradRotKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                              const scalar* __restrict__ w, const scalar* __restrict__ fT, int comp,
                              const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz,
                              const scalar* __restrict__ Sfx, const scalar* __restrict__ Sfy, const scalar* __restrict__ Sfz,
                              const scalar* __restrict__ V, scalar* __restrict__ gx, scalar* __restrict__ gy, scalar* __restrict__ gz) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const label o = own[j], nb = nbr[j];
    scalar rx, ry, rz; rotNbr(fT, n, j, Ux[nb], Uy[nb], Uz[nb], rx, ry, rz);
...
__global__ void deferredRotKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                                  const scalar* __restrict__ ifc, const scalar* __restrict__ fT, int comp,
                                  const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz,
...
__global__ void addHDiagKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
                               const scalar* __restrict__ ifcC, const scalar* __restrict__ psi,
                               const scalar* __restrict__ V, scalar* __restrict__ H) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
...
void deviceCyclicAssembleLaplacian(DeviceCyclic& cyc, const DeviceBuffer<scalar>& gammaCell,
                                   DeviceBuffer<scalar>& diag, bool addToDiag) {
    if (cyc.n == 0) return;
...
namespace { __global__ void cycZeroWallKernel(int n, const label* __restrict__ own, const label* __restrict__ isW, scalar* __restrict__ ifc) {
    const int j = blockIdx.x*blockDim.x+threadIdx.x; if (j<n && isW[own[j]]) ifc[j]=0.0; } }
...
__global__ void cycLinUpwindKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
    ...
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const int o = own[j], nb = nbr[j]; const scalar pf = phi[j]; scalar c;
...
__global__ void cycLapCorrKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
        const scalar* __restrict__ nu, const scalar* __restrict__ w, const scalar* __restrict__ magSf,
    ...
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= n) return;
    const int o = own[j], nb = nbr[j]; const scalar wf = w[j], wn = 1.0 - wf;
    ...
    if (rotational) {                                  // grad((R·U)_comp) = (R · gradU[nbr] · Rᵀ)[comp][:]
        const scalar G[9] = { gx0[nb],gy0[nb],gz0[nb], gx1[nb],gy1[nb],gz1[nb], gx2[nb],gy2[nb],gz2[nb] };
        scalar R[9]; for (int q = 0; q < 9; ++q) R[q] = fT[q*n + j];
        scalar gn[3];
        for (int jj = 0; jj < 3; ++jj) { scalar s = 0;
            for (int i = 0; i < 3; ++i) for (int k = 0; k < 3; ++k) s += R[3*comp+i]*G[3*i+k]*R[3*jj+k];
            gn[jj] = s; }
...
__global__ void cycLapCorrScalarKernel(int n, const label* __restrict__ own, const label* __restrict__ nbr,
        const scalar* __restrict__ gamma, const scalar* __restrict__ w, const scalar* __restrict__ magSf,
        const scalar* __restrict__ cvx, const scalar* __restrict__ cvy, const scalar* __restrict__ cvz,
...
void deviceCyclicLapCorrP(const DeviceCyclic& cyc, const DeviceBuffer<scalar>& gammaCell,
    ...
    if (cyc.n == 0) return; ffcOut.resize(cyc.n);
