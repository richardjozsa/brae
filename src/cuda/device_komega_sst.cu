// cf GPU offload (C③): k-omega SST blending (F1/F2), cross-diffusion (CDkOmega), strain (S2), and the
// coefficient blend. Pointwise per-cell kernels mirroring kOmegaSSTBase.C (v2412) exactly. See the header for
// the formulas. Validated vs OF on a frozen field state (ctest gpu_komega_sst).
...
__global__ void s2Kernel(int nC, const scalar* __restrict__ gradU, scalar* __restrict__ S2) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar t[9]; for (int q = 0; q < 9; ++q) t[q] = gradU[q * nC + c];
    scalar s = 0.0;                                            // magSqr(symm(gradU)) = sum_ab symm_ab^2
    for (int a = 0; a < 3; ++a)
        for (int b = 0; b < 3; ++b) { const scalar sab = 0.5 * (t[a*3+b] + t[b*3+a]); s += sab * sab; }
    S2[c] = 2.0 * s;
}
__global__ void cdKernel(int nC, const scalar* __restrict__ gKx, const scalar* __restrict__ gKy, const scalar* __restrict__ gKz,
                         const scalar* __restrict__ gOx, const scalar* __restrict__ gOy, const scalar* __restrict__ gOz,
                         const scalar* __restrict__ om, scalar twoA2, scalar* __restrict__ CD) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    const scalar dot = gKx[c]*gOx[c] + gKy[c]*gOy[c] + gKz[c]*gOz[c];   // grad k . grad omega
    CD[c] = twoA2 * dot / om[c];
}
__global__ void f1Kernel(int nC, const scalar* __restrict__ k, const scalar* __restrict__ om, const scalar* __restrict__ y,
                         const scalar* __restrict__ CD, scalar nu, scalar betaStar, scalar alphaOmega2, int lm, scalar* __restrict__ F1) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    ...
    if (lm) {
        const scalar Ry = yy * sqrt(kk) / nu; const scalar r = Ry / 120.0;
        const scalar r2 = r * r, r4 = r2 * r2;
        f1 = fmax(f1, exp(-(r4 * r4)));
    }
    F1[c] = f1;
}
__global__ void f2Kernel(int nC, const scalar* __restrict__ k, const scalar* __restrict__ om, const scalar* __restrict__ y,
                         scalar nu, scalar betaStar, scalar* __restrict__ F2) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    const scalar kk = k[c], w = om[c], yy = y[c];
...
__global__ void blendKernel(int nC, const scalar* __restrict__ F1, scalar psi1, scalar psi2, scalar* __restrict__ out) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c < nC) out[c] = F1[c] * (psi1 - psi2) + psi2;
}
// correctNut: nut = a1*k / max(a1*omega, b1*F2*sqrt(S2))
__global__ void nutSSTKernel(int nC, const scalar* __restrict__ k, const scalar* __restrict__ om,
                             const scalar* __restrict__ F2, const scalar* __restrict__ S2,
                             scalar a1, scalar b1, scalar* __restrict__ nut) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
...
__global__ void pkKernel(int nC, const scalar* __restrict__ G, const scalar* __restrict__ k, const scalar* __restrict__ om,
                         scalar c1betaStar, scalar* __restrict__ Pk) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    Pk[c] = fmin(G[c], c1betaStar * k[c] * om[c]);
}
// GbyNu = min(GbyNu0, (c1/a1)*betaStar*omega*max(a1*omega, b1*F2*sqrt(S2)))
__global__ void gbyNuLimitKernel(int nC, const scalar* __restrict__ GbyNu0, const scalar* __restrict__ om,
                                 const scalar* __restrict__ F2, const scalar* __restrict__ S2,
                                 scalar a1, scalar b1, scalar c1, scalar betaStar, scalar* __restrict__ GbyNu) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    const scalar denom = fmax(a1 * om[c], b1 * F2[c] * sqrt(S2[c]));
...
__global__ void dEffKernel(int nC, const scalar* __restrict__ F1, const scalar* __restrict__ nut,
                           scalar alpha1, scalar alpha2, scalar nu, scalar* __restrict__ D) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
...
__global__ void omegaReactionKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ gamma,
                                    const scalar* __restrict__ beta, const scalar* __restrict__ GbyNu0,
                                    const scalar* __restrict__ F1, const scalar* __restrict__ CD,
                                    const scalar* __restrict__ om, const scalar* __restrict__ divU,
...
__global__ void kReactionSSTKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ k,
                                   const scalar* __restrict__ om, const scalar* __restrict__ G,
                                   const scalar* __restrict__ divU, scalar betaStar, scalar c1betaStar,
...
__global__ void wallOmegaG0Kernel(int nWF, const label* __restrict__ wfCell, const scalar* __restrict__ wfY,
                                  ...
                                  scalar nu, scalar yplLam, scalar Cmu25, scalar kappa, scalar E, scalar beta1,
                                  scalar* __restrict__ omega0, scalar* __restrict__ G0) {
...
void deviceCDkOmega(const DeviceBuffer<scalar>& gKx, const DeviceBuffer<scalar>& gKy, const DeviceBuffer<scalar>& gKz,
    ...
    const int nC = static_cast<int>(omega.size()); CD.resize(nC);
...
void deviceF1(const DeviceBuffer<scalar>& k, const DeviceBuffer<scalar>& omega, const DeviceBuffer<scalar>& y,
              const DeviceBuffer<scalar>& CD, scalar nu, const KOmegaSSTCoeffs& co, DeviceBuffer<scalar>& F1, bool lm) {
    const int nC = static_cast<int>(k.size()); F1.resize(nC);
...
void deviceF2(const DeviceBuffer<scalar>& k, const DeviceBuffer<scalar>& omega, const DeviceBuffer<scalar>& y,
    ...
    const int nC = static_cast<int>(k.size()); F2.resize(nC);
...
void deviceBlend(const DeviceBuffer<scalar>& F1, scalar psi1, scalar psi2, DeviceBuffer<scalar>& out) {
    const int nC = static_cast<int>(F1.size()); out.resize(nC);
...
void devicePk(const DeviceBuffer<scalar>& G, const DeviceBuffer<scalar>& k, const DeviceBuffer<scalar>& omega,
    ...
    const int nC = static_cast<int>(k.size()); Pk.resize(nC);
...
void deviceGbyNuLimit(const DeviceBuffer<scalar>& GbyNu0, const DeviceBuffer<scalar>& omega,
    ...
    const int nC = static_cast<int>(omega.size()); GbyNu.resize(nC);
...
void deviceDEff(const DeviceBuffer<scalar>& F1, const DeviceBuffer<scalar>& nut, scalar alpha1, scalar alpha2,
    ...
    const int nC = static_cast<int>(F1.size()); D.resize(nC);
