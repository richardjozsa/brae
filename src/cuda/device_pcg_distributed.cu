// Distributed (multi-GPU) Krylov solvers: the deviceParallel* twins of the single-GPU PCG/BiCGStab -- A*x via
// deviceParallelAmul (halo exchange) and every dot / sumMag a GLOBAL reduction (on-stream NVSHMEM DeviceReducer).
// Includes the distributed AMG-PCG (aggregation-based coarse Galerkin -> aggSumK/gal*K) + the CUDA>=13 device-resident
// whole-loop BiCGStab graph. Verbatim split of device_pcg.cu -- no logic change. (parallel_* FILES untouched.)
#include "device_pcg.cuh"
#include "device_pcg_detail.cuh"  // omegaK/bicg*K + BiCGScalars/bicgScalars (shared)
#include "device_amg.cuh"        // AMGData + amgVCycleApply (distributed AMG-PCG preconditioner)
#include "device_blas.cuh"
#include "device_ldu.cuh"        // deviceParallelAmul
#include "device_halo.cuh"
#include "device_reduce.cuh"     // DeviceReducer (on-stream NVSHMEM reduce)
