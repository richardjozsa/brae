...
inline bool envFlag(const char* name, bool def) {          // default-ON flag: def unless explicitly 0/false/off/no
...
template <typename T> __global__ void zeroT(int n, T* x) {
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
    if (i < n) x[i] = T(0);
}
__global__ void fillK(int n, scalar* x, scalar v) {
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
    if (i < n) x[i] = v;
}
...
__global__ void tsgsZUpdateK(int n, const scalar* __restrict__ b, const scalar* __restrict__ Ax, const scalar* __restrict__ diag,
    ...
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
    if (i < n) { const scalar zi = (b[i]-Ax[i])/diag[i]; z[i]=zi; x[i]+=zi; }
}
...
static scalar deviceSymGaussSeidelF32(const DeviceLduView&, const DeviceBuffer<scalar>&, DeviceBuffer<scalar>&,
                                      scalar, scalar, scalar, int);   // FP32 turbulence GS (defined below)
...
template <class S, class D> __global__ void cast_(int n, const S* __restrict__ s, D* __restrict__ d) {
    const int i = blockIdx.x*blockDim.x + threadIdx.x;
    if (i < n) d[i] = (D)s[i];
}
...
DeviceSolverPerf deviceAMGPCG(const DeviceLduView& A, AMGData& amg, const DeviceBuffer<scalar>& b,
    ...
    if (!converged(perf.finalResidual)) {
        do {
            ...
            else { deviceScalarDiv(dWArA, dWArAold, dBeta);     // Fletcher-Reeves beta
                   deviceFusedScaleAxpy(pA, dBeta, wA); }
