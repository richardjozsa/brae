...
__global__ void interpKernel(int nIf, const label* __restrict__ own, const label* __restrict__ nei,
                             const scalar* __restrict__ w, const scalar* __restrict__ vol, scalar* __restrict__ sf) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f < nIf) sf[f] = w[f] * vol[own[f]] + (1.0 - w[f]) * vol[nei[f]];
}

__global__ void divKernel(int nC, const label* __restrict__ ownerStart, const label* __restrict__ losort,
                          const label* __restrict__ losortStart, const scalar* __restrict__ phiInt,
                          const label* __restrict__ bndCellStart, const label* __restrict__ bndPerm,
                          const scalar* __restrict__ bval, const scalar* __restrict__ V, scalar* __restrict__ d) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar s = 0.0;
    for (int f = ownerStart[c]; f < ownerStart[c + 1]; ++f) s += phiInt[f];              // +owner internal
    for (int k = losortStart[c]; k < losortStart[c + 1]; ++k) s -= phiInt[losort[k]];    // -neighbour internal
    for (int k = bndCellStart[c]; k < bndCellStart[c + 1]; ++k) s += bval[bndPerm[k]];   // +boundary
    d[c] = s / V[c];
}

__global__ void gradKernel(int nC, const label* __restrict__ own, const label* __restrict__ nei,
                           const scalar* __restrict__ w, const scalar* __restrict__ Sfx,
                           const scalar* __restrict__ Sfy, const scalar* __restrict__ Sfz,
                           const scalar* __restrict__ vol, const label* __restrict__ ownerStart,
                           const label* __restrict__ losort, const label* __restrict__ losortStart,
                           const label* __restrict__ bndCellStart, const label* __restrict__ bndPerm,
                           const label* __restrict__ bndGFace, const scalar* __restrict__ bval,
    ...
    for (int f = ownerStart[c]; f < ownerStart[c + 1]; ++f) {                            // +owner internal
        ...
        sx += Sfx[f] * pf; sy += Sfy[f] * pf; sz += Sfz[f] * pf;
    }
    ...
    for (int k = bndCellStart[c]; k < bndCellStart[c + 1]; ++k) {                        // +boundary
        const int kk = bndPerm[k]; const int f = bndGFace[kk]; const scalar pv = bval[kk];
        sx += Sfx[f] * pv; sy += Sfy[f] * pv; sz += Sfz[f] * pv;
    }
    gx[c] = sx / V[c]; gy[c] = sy / V[c]; gz[c] = sz / V[c];
}
} // namespace
...
void deviceGaussGrad(const DeviceMesh& dm, const DeviceBuffer<scalar>& vol, const DeviceBuffer<scalar>& bval,
    ...
    gx.resize(dm.nCells); gy.resize(dm.nCells); gz.resize(dm.nCells);
...
__device__ __forceinline__ scalar limFace(scalar maxD, scalar minD, scalar ex) {
    if (ex >  1e-15) return fmin(maxD / ex, 1.0);
    if (ex < -1e-15) return fmin(minD / ex, 1.0);
    return 1.0;
}
__global__ void cellLimitGradKernel(int nC, scalar k,
        const scalar* __restrict__ U, const scalar* __restrict__ Ubnd,
        const label* __restrict__ ownerStart, const label* __restrict__ nei,
        const label* __restrict__ losort, const label* __restrict__ losortStart, const label* __restrict__ owner,
        const label* __restrict__ bndCellStart, const label* __restrict__ bndPerm,
    ...
    for (int f = ownerStart[c]; f < ownerStart[c+1]; ++f)  { const scalar d = U[nei[f]]          - uc; maxD = fmax(maxD,d); minD = fmin(minD,d); }
    for (int j = losortStart[c]; j < losortStart[c+1]; ++j){ const scalar d = U[owner[losort[j]]] - uc; maxD = fmax(maxD,d); minD = fmin(minD,d); }
    for (int j = bndCellStart[c]; j < bndCellStart[c+1]; ++j){ const scalar d = Ubnd[bndPerm[j]]  - uc; maxD = fmax(maxD,d); minD = fmin(minD,d); }
    if (k < 1.0) { const scalar wdn = (1.0/k - 1.0)*(maxD - minD); maxD += wdn; minD -= wdn; }   // OF k<1 widening
    const scalar gcx = gx[c], gcy = gy[c], gcz = gz[c];
    scalar lim = 1.0;
    for (int f = ownerStart[c]; f < ownerStart[c+1]; ++f)   lim = fmin(lim, limFace(maxD, minD, dOwnX[f]*gcx + dOwnY[f]*gcy + dOwnZ[f]*gcz));
    for (int j = losortStart[c]; j < losortStart[c+1]; ++j){ const int f = losort[j]; lim = fmin(lim, limFace(maxD, minD, dNeiX[f]*gcx + dNeiY[f]*gcy + dNeiZ[f]*gcz)); }
    for (int j = bndCellStart[c]; j < bndCellStart[c+1]; ++j){ const int bk = bndPerm[j]; lim = fmin(lim, limFace(maxD, minD, dBndX[bk]*gcx + dBndY[bk]*gcy + dBndZ[bk]*gcz)); }
    gx[c] = gcx*lim; gy[c] = gcy*lim; gz[c] = gcz*lim;
}
} // namespace
