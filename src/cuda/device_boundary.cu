...
__global__ void bcValueKernel(int n, const label* __restrict__ type, const scalar* __restrict__ ref,
                              const scalar* __restrict__ vf, const label* __restrict__ fc,
                              const scalar* __restrict__ internal, scalar* __restrict__ value) {
...
__global__ void bcLaplacianKernel(int n, const label* __restrict__ type, const scalar* __restrict__ ref,
                                  const scalar* __restrict__ vf, const scalar* __restrict__ dc,
                                  const scalar* __restrict__ magSf, const scalar* __restrict__ gammaCell,
                                  const label* __restrict__ fc, scalar* __restrict__ iC, scalar* __restrict__ bC) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    const scalar pG = gammaCell[fc[i]] * magSf[i];                        // pGamma = gammaf_b * |Sf|
    const scalar w = (type[i] == 1) ? 1.0 : ((type[i] == 5) ? vf[i] : 0.0);
    iC[i] = pG * (-w * dc[i]); bC[i] = -pG * (w * ref[i] * dc[i]);        // gradIC=-vf*dc, gradBC=vf*ref*dc
}
// same as bcLaplacianKernel but gamma is given per BOUNDARY FACE (e.g. nuEff = nu + nutkWallFunction at walls).
__global__ void bcLaplacianFaceKernel(int n, const label* __restrict__ type, const scalar* __restrict__ ref,
                                      const scalar* __restrict__ vf, const scalar* __restrict__ dc,
                                      const scalar* __restrict__ magSf, const scalar* __restrict__ gammaFace,
                                      scalar* __restrict__ iC, scalar* __restrict__ bC) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    const scalar pG = gammaFace[i] * magSf[i];
    const scalar w = (type[i] == 1) ? 1.0 : ((type[i] == 5) ? vf[i] : 0.0);
    iC[i] = pG * (-w * dc[i]); bC[i] = -pG * (w * ref[i] * dc[i]);
}
__global__ void bcDivKernel(int n, const label* __restrict__ type, const scalar* __restrict__ ref,
                            const scalar* __restrict__ vf, const scalar* __restrict__ phiB,
                            scalar* __restrict__ iC, scalar* __restrict__ bC) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    ...
    iC[i] = phiB[i] * vIC; bC[i] = -phiB[i] * vBC;
}
__global__ void bcMatrixFluxKernel(int n, const label* __restrict__ fc, const scalar* __restrict__ iC,
                                   const scalar* __restrict__ bC, const scalar* __restrict__ p, scalar* __restrict__ fluxB) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void ioUpdateKernel(int n, const label* __restrict__ ioMask, const label* __restrict__ oioMask,
                               const scalar* __restrict__ phiB, label* __restrict__ bcType) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
...
__global__ void mixedUpdateKernel(int n, const label* __restrict__ maskU, const label* __restrict__ maskP,
                                  const label* __restrict__ fc, const scalar* __restrict__ phiB,
                                  const scalar* __restrict__ magSf, const scalar* __restrict__ Ux,
                                  const scalar* __restrict__ Uy, const scalar* __restrict__ Uz,
                                  scalar* __restrict__ vfU0, scalar* __restrict__ vfU1, scalar* __restrict__ vfU2,
    ...
    if (mu) { const scalar vfu = 0.5 - 0.5 * ct; vfU0[i] = vfu; vfU1[i] = vfu; vfU2[i] = vfu; }   // velocity sign
...
__global__ void piovUpdateKernel(int n, const label* __restrict__ piov, const label* __restrict__ fc,
        const scalar* __restrict__ phiB, const scalar* __restrict__ nx, const scalar* __restrict__ ny,
        const scalar* __restrict__ nz, const scalar* __restrict__ Ux, const scalar* __restrict__ Uy,
        const scalar* __restrict__ Uz, label* __restrict__ ty0, label* __restrict__ ty1, label* __restrict__ ty2,
        scalar* __restrict__ r0, scalar* __restrict__ r1, scalar* __restrict__ r2) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n || !piov[i]) return;
    if (phiB[i] >= 0.0) { ty0[i] = ty1[i] = ty2[i] = 0; }                        // outflow -> zeroGradient
    else {                                                                       // inflow -> fixedValue normal projection
        const int c = fc[i];
        const scalar Un = nx[i] * Ux[c] + ny[i] * Uy[c] + nz[i] * Uz[c];         // n . U_cell
        ty0[i] = ty1[i] = ty2[i] = 1;
        r0[i] = nx[i] * Un; r1[i] = ny[i] * Un; r2[i] = nz[i] * Un;
    }
}
...
__global__ void symUpdateKernel(int n, const label* __restrict__ sym, const label* __restrict__ fc,
        ...
        scalar* __restrict__ vf0, scalar* __restrict__ vf1, scalar* __restrict__ vf2,
        scalar* __restrict__ r0, scalar* __restrict__ r1, scalar* __restrict__ r2) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n || !sym[i]) return;
    const int c = fc[i];
    ...
    vf0[i] = fabs(nx[i]); vf1[i] = fabs(ny[i]); vf2[i] = fabs(nz[i]);
    r0[i] = Ux[c] - sgx * nU; r1[i] = Uy[c] - sgy * nU; r2[i] = Uz[c] - sgz * nU;
}
...
__global__ void symHbyAKernel(int n, const label* __restrict__ sym, const label* __restrict__ fc,
        const scalar* __restrict__ nx, const scalar* __restrict__ ny, const scalar* __restrict__ nz,
        const scalar* __restrict__ Hx, const scalar* __restrict__ Hy, const scalar* __restrict__ Hz,
        scalar* __restrict__ hxb, scalar* __restrict__ hyb, scalar* __restrict__ hzb) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n || !sym[i]) return;
    const int c = fc[i];
    const scalar nH = nx[i] * Hx[c] + ny[i] * Hy[c] + nz[i] * Hz[c];
    hxb[i] = Hx[c] - nx[i] * nH; hyb[i] = Hy[c] - ny[i] * nH; hzb[i] = Hz[c] - nz[i] * nH;
}
// at mixed faces, overwrite the HbyA boundary value with the U boundary value (constrainHbyA at fixesValue patches).
__global__ void selectMixedKernel(int n, const label* __restrict__ mask, const scalar* __restrict__ ux,
                                  const scalar* __restrict__ uy, const scalar* __restrict__ uz,
                                  scalar* __restrict__ hx, scalar* __restrict__ hy, scalar* __restrict__ hz) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void deviceUpdateMixedFreestream(DeviceVectorBoundary& dbU, DeviceBoundary& dbP, const DeviceBuffer<scalar>& phiBnd,
    ...
    const int n = dbP.n; if (n == 0) return;
...
void deviceUpdatePressureInletOutletVelocity(DeviceVectorBoundary& dbU, const DeviceBuffer<scalar>& phiBnd,
    ...
    const int n = dbU.n; if (n == 0) return;
...
void deviceConstrainMixedHbyA(const DeviceVectorBoundary& dbU, const DeviceBuffer<scalar>& Ux,
    ...
    deviceBCValue(dbU.comp[0], Ux, ubx); deviceBCValue(dbU.comp[1], Uy, uby); deviceBCValue(dbU.comp[2], Uz, ubz);
...
__global__ void tpUpdateKernel(int n, const label* __restrict__ tpMask, const scalar* __restrict__ p0,
                               const scalar* __restrict__ phiB, const scalar* __restrict__ Uxb,
                               const scalar* __restrict__ Uyb, const scalar* __restrict__ Uzb, scalar* __restrict__ refValue) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
