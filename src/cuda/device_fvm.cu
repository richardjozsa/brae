...
__global__ void lapFaceKernel(int nIf, const scalar* __restrict__ dc, const scalar* __restrict__ gammaf,
                              const scalar* __restrict__ magSf, scalar* __restrict__ upper, scalar* __restrict__ lower) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f < nIf) { const scalar c = dc[f] * gammaf[f] * magSf[f]; upper[f] = c; lower[f] = c; }
}
__global__ void divFaceKernel(int nIf, const scalar* __restrict__ phi, scalar* __restrict__ upper, scalar* __restrict__ lower) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f < nIf) { const scalar p = phi[f]; const scalar w = (p >= 0.0) ? 1.0 : 0.0; const scalar lo = -w * p; lower[f] = lo; upper[f] = lo + p; }
}
...
__global__ void divLimitedFaceKernel(int nIf, const label* __restrict__ own, const label* __restrict__ nei,
                                     const scalar* __restrict__ cdw, const scalar* __restrict__ phi, const scalar* __restrict__ field,
                                     ...
                                     scalar twoByk, scalar* __restrict__ upper, scalar* __restrict__ lower) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f >= nIf) return;
    ...
    const scalar lo = -W * p; lower[f] = lo; upper[f] = lo + p;
}
__global__ void diagGatherKernel(int nC, const label* __restrict__ ownerStart, const label* __restrict__ losort,
                                 const label* __restrict__ losortStart, const scalar* __restrict__ upper,
                                 const scalar* __restrict__ lower, scalar* __restrict__ diag) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar s = 0.0;
    for (int f = ownerStart[c]; f < ownerStart[c + 1]; ++f) s += lower[f];              // faces owned by c
    for (int k = losortStart[c]; k < losortStart[c + 1]; ++k) s += upper[losort[k]];    // faces neighbouring c
    diag[c] = -s;
}
...
__global__ void foldPressureKernel(int nC, const label* __restrict__ bndCellStart, const label* __restrict__ bndPerm,
                                   const scalar* __restrict__ rawDiag, const scalar* __restrict__ V,
                                   const scalar* __restrict__ divPhi, const scalar* __restrict__ iC,
                                   const scalar* __restrict__ bC, scalar* __restrict__ diagC, scalar* __restrict__ b) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar dd = rawDiag[c], bb = V[c] * divPhi[c];
    for (int k = bndCellStart[c]; k < bndCellStart[c + 1]; ++k) { const int kk = bndPerm[k]; dd += iC[kk]; bb += bC[kk]; }
    diagC[c] = dd; b[c] = bb;
}
} // namespace
...
void deviceLaplacianCoeffs(const DeviceMesh& dm, const DeviceBuffer<scalar>& gammafInt,
    ...
    upper.resize(nIf); lower.resize(nIf); diag.resize(nC);
...
__global__ void lapCorrFaceKernel(int nIf, const label* __restrict__ own, const label* __restrict__ nei,
                                  const scalar* __restrict__ w, const scalar* __restrict__ gammaf, const scalar* __restrict__ magSf,
                                  const scalar* __restrict__ cvx, const scalar* __restrict__ cvy, const scalar* __restrict__ cvz,
                                  const scalar* __restrict__ gx, const scalar* __restrict__ gy, const scalar* __restrict__ gz,
                                  scalar* __restrict__ ffc) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f >= nIf) return;
    const int o = own[f], n = nei[f]; const scalar wf = w[f], wn = 1.0 - wf;
...
__global__ void lapCorrFaceLimitedKernel(int nIf, const label* __restrict__ own, const label* __restrict__ nei,
                                         const scalar* __restrict__ w, const scalar* __restrict__ gammaf, const scalar* __restrict__ magSf,
                                         const scalar* __restrict__ cvx, const scalar* __restrict__ cvy, const scalar* __restrict__ cvz,
                                         const scalar* __restrict__ nonOrthDc, const scalar* __restrict__ phi,
                                         const scalar* __restrict__ gx, const scalar* __restrict__ gy, const scalar* __restrict__ gz,
                                         scalar psi, scalar* __restrict__ ffc) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f >= nIf) return;
    const int o = own[f], n = nei[f]; const scalar wf = w[f], wn = 1.0 - wf;
    const scalar gfx = wf*gx[o] + wn*gx[n], gfy = wf*gy[o] + wn*gy[n], gfz = wf*gz[o] + wn*gz[n];
...
__global__ void lapCorrGatherKernel(int nC, const label* __restrict__ ownerStart, const label* __restrict__ losort,
                                    const label* __restrict__ losortStart, const scalar* __restrict__ ffc, scalar* __restrict__ src) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar s = 0.0;
    for (int f = ownerStart[c]; f < ownerStart[c + 1]; ++f) s -= ffc[f];               // c is owner
    for (int k = losortStart[c]; k < losortStart[c + 1]; ++k) s += ffc[losort[k]];      // c is neighbour
    src[c] = s;
}
} // namespace
...
void deviceLaplacianCorrFlux(const DeviceMesh& dm, const DeviceBuffer<scalar>& gammafInt,
    ...
    const int nIf = dm.nInternalFaces; ffc.resize(nIf);
...
// (bit-identical). phi = the scalar field being corrected (for the orthogonal snGrad reference).
void deviceLaplacianCorrFluxLimited(const DeviceMesh& dm, const DeviceBuffer<scalar>& gammafInt, const DeviceBuffer<scalar>& phi,
    ...
    if (psi >= 1.0) { deviceLaplacianCorrFlux(dm, gammafInt, gx, gy, gz, ffc); return; }
    const int nIf = dm.nInternalFaces; ffc.resize(nIf);
...
void deviceFaceDivSource(const DeviceMesh& dm, const DeviceBuffer<scalar>& ffc, DeviceBuffer<scalar>& src) {
    const int nC = dm.nCells; src.resize(nC);
...
void deviceLaplacianCorr(const DeviceMesh& dm, const DeviceBuffer<scalar>& gammafInt,
    ...
    DeviceBuffer<scalar> ffc; deviceLaplacianCorrFlux(dm, gammafInt, gx, gy, gz, ffc);
...
void deviceFoldPressure(const DeviceMesh& dm, const DeviceBuffer<scalar>& rawDiag, const DeviceBuffer<scalar>& divPhi,
    ...
    const int nC = dm.nCells; diagC.resize(nC); b.resize(nC);
...
namespace {
__global__ void foldKernel(int nC, const label* __restrict__ bndCellStart, const label* __restrict__ bndPerm,
                           const scalar* __restrict__ rawDiag, const scalar* __restrict__ source,
                           const scalar* __restrict__ iC, const scalar* __restrict__ bC,
                           scalar* __restrict__ diagC, scalar* __restrict__ b) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
