...
__global__ void amulKernel(int nC,
                           const scalar* __restrict__ diag, const scalar* __restrict__ upper,
                           const scalar* __restrict__ lower, const label* __restrict__ nei,
                           const label* __restrict__ owner, const label* __restrict__ ownerStart,
                           const label* __restrict__ losort, const label* __restrict__ losortStart,
                           const scalar* __restrict__ psi, scalar* __restrict__ Apsi) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar s = diag[c] * psi[c];
    const int u0 = ownerStart[c], u1 = ownerStart[c + 1];
    for (int f = u0; f < u1; ++f) s += upper[f] * psi[nei[f]];          // faces owned by c
    const int l0 = losortStart[c], l1 = losortStart[c + 1];
    for (int k = l0; k < l1; ++k) { const int f = losort[k]; s += lower[f] * psi[owner[f]]; }  // faces neighbouring c
    Apsi[c] = s;
}
...
__global__ void cyclicAmulKernel(int nCyc, const label* __restrict__ own, const label* __restrict__ nbr,
                                 const scalar* __restrict__ coeff, const scalar* __restrict__ psi,
                                 scalar* __restrict__ Apsi) {
    const int j = blockIdx.x * blockDim.x + threadIdx.x; if (j >= nCyc) return;
...
__global__ void amiAmulKernel(int n, const label* __restrict__ own, const label* __restrict__ off,
    ...
    scalar s = 0; for (label k = off[i]; k < off[i+1]; ++k) s += w[k] * psi[nbr[k]];
