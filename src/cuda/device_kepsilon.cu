// cf GPU offload (#3): k-epsilon production + eddy viscosity. gradU is the OF-convention tensor (column i =
...
__global__ void gByNuKernel(int nC, const scalar* __restrict__ gradU, scalar* __restrict__ gByNu) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
    scalar t[9]; for (int q = 0; q < 9; ++q) t[q] = gradU[q * nC + c];
...
__global__ void nutKernel(int nC, const scalar* __restrict__ k, const scalar* __restrict__ eps, scalar Cmu, scalar* __restrict__ nut) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void rkeStrainKernel(int nC, const scalar* __restrict__ gradU, const scalar* __restrict__ k,
                                const scalar* __restrict__ eps, scalar A0, scalar* __restrict__ rCmu, scalar* __restrict__ magS) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    scalar t[9]; for (int q = 0; q < 9; ++q) t[q] = gradU[q * nC + c];
    const scalar third = (t[0] + t[4] + t[8]) / 3.0;
    scalar S[9];
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) S[i*3+j] = 0.5*(t[i*3+j] + t[j*3+i]) - ((i==j) ? third : 0.0);
    scalar magSqrS = 0.0, skSq = 0.0, tr3 = 0.0;
    for (int q = 0; q < 9; ++q) magSqrS += S[q]*S[q];
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) { const scalar sk = 0.5*(t[i*3+j]-t[j*3+i]); skSq += sk*sk; }
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) for (int m = 0; m < 3; ++m) tr3 += S[i*3+j]*S[j*3+m]*S[m*3+i];
...
__global__ void rkeNutKernel(int nC, const scalar* __restrict__ rCmu, const scalar* __restrict__ k,
                             const scalar* __restrict__ eps, scalar* __restrict__ nut) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void rkeEpsReactionKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ eps,
                                     const scalar* __restrict__ k, const scalar* __restrict__ magS, scalar nu, scalar C2,
...
__global__ void wallFnKernel(int nWF, const label* __restrict__ wfCell, const scalar* __restrict__ wfY,
                             const scalar* __restrict__ wfDc, const scalar* __restrict__ wux, const scalar* __restrict__ wuy,
                             const scalar* __restrict__ wuz, const scalar* __restrict__ invNw, const scalar* __restrict__ k,
                             const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz,
                             scalar nu, scalar yplLam, scalar Cmu25, scalar Cmu75, scalar kappa, scalar E,
                             scalar* __restrict__ eps0, scalar* __restrict__ G0) {
    const int wf = blockIdx.x * blockDim.x + threadIdx.x;
    if (wf >= nWF) return;
    const int c = wfCell[wf]; const scalar y = wfY[wf], dc = wfDc[wf], kc = k[c];
...
__global__ void boundaryNutKernel(int n, const label* __restrict__ fc, const label* __restrict__ isWall,
                                  const scalar* __restrict__ y, const scalar* __restrict__ k, const scalar* __restrict__ nut,
                                  scalar nu, scalar yplLam, scalar Cmu25, scalar kappa, scalar E, scalar* __restrict__ nutBnd) {
...
__global__ void epsReactionKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ eps,
                                  const scalar* __restrict__ k, const scalar* __restrict__ gByNu,
                                  const scalar* __restrict__ divU, scalar C1, scalar C2, scalar C3, scalar Cmu,
                                  scalar* __restrict__ diag, scalar* __restrict__ source) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
...
__global__ void kReactionKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ k,
                                const scalar* __restrict__ eps, const scalar* __restrict__ G,
                                const scalar* __restrict__ divU, scalar* __restrict__ diag, scalar* __restrict__ source) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x;
    if (c >= nC) return;
...
__global__ void depsKernel(int nC, const scalar* __restrict__ nut, scalar sigma, scalar nu, scalar* __restrict__ D) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC) D[c] = nut[c] / sigma + nu;
}
__global__ void overrideKernel(int nC, const label* __restrict__ isW, const scalar* __restrict__ G0,
                               const scalar* __restrict__ eps0, scalar* __restrict__ G, scalar* __restrict__ eps) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC && isW[c]) { G[c] = G0[c]; eps[c] = eps0[c]; }
}
...
__global__ void boundClampKernel(int nC, scalar floor, const scalar* __restrict__ x, scalar* __restrict__ cl) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC) cl[c] = fmax(x[c], floor);
}
__global__ void boundAvgGatherKernel(int nC, const label* __restrict__ ownerStart, const label* __restrict__ losort,
                                     const label* __restrict__ losortStart, const label* __restrict__ bndCellStart,
                                     const scalar* __restrict__ faceInterp, const scalar* __restrict__ cl, scalar* __restrict__ avg) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    scalar s = 0.0; int nf = 0;
...
__global__ void boundApplyKernel(int nC, scalar floor, const scalar* __restrict__ avg, scalar* __restrict__ x) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
...
__global__ void svFaceKernel(int nIf, const label* __restrict__ own, const label* __restrict__ nei,
                             const label* __restrict__ isW, const scalar* __restrict__ eps0,
                             scalar* __restrict__ upper, scalar* __restrict__ lower, scalar* __restrict__ source) {
    const int f = blockIdx.x * blockDim.x + threadIdx.x;
    if (f >= nIf) return;
    const int o = own[f], n = nei[f]; const bool ow = isW[o], nw = isW[n];
    if (ow) atomicAdd(&source[n], -lower[f] * eps0[o]);
    if (nw) atomicAdd(&source[o], -upper[f] * eps0[n]);
    if (ow || nw) { upper[f] = 0.0; lower[f] = 0.0; }
}
__global__ void svBndKernel(int nB, const label* __restrict__ faceCell, const label* __restrict__ isW,
                            scalar* __restrict__ iC, scalar* __restrict__ bC) {
    const int bi = blockIdx.x * blockDim.x + threadIdx.x; if (bi < nB && isW[faceCell[bi]]) { iC[bi] = 0.0; bC[bi] = 0.0; }
}
__global__ void svCellKernel(int nC, const label* __restrict__ isW, const scalar* __restrict__ relaxedDiag,
                             const scalar* __restrict__ eps0, scalar* __restrict__ source) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC && isW[c]) source[c] = relaxedDiag[c] * eps0[c];
}
} // namespace
...
void deviceGradU(const DeviceMesh& dm, const DeviceVectorBoundary& dbU,
                 ...
                 DeviceBuffer<scalar>& gradU, DeviceAMI* ami, DeviceCyclic* cyc) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // ASYNC D2D on the per-thread stream (ordered before the consumer kernel): a plain cudaMemcpy here drains
        // the GPU pipeline 9×/turbulence-iter (the stall pattern deviceCopy was rewritten to avoid).
...
void deviceGbyNu(const DeviceMesh& dm, const DeviceVectorBoundary& dbU,
                 const DeviceBuffer<scalar>& Ux, const DeviceBuffer<scalar>& Uy, const DeviceBuffer<scalar>& Uz,
                 DeviceBuffer<scalar>& gByNu, DeviceAMI* ami, DeviceCyclic* cyc) {
    DeviceBuffer<scalar> gradU; deviceGradU(dm, dbU, Ux, Uy, Uz, gradU, ami, cyc);
...
void deviceNut(const DeviceBuffer<scalar>& k, const DeviceBuffer<scalar>& eps, DeviceBuffer<scalar>& nut,
    ...
    const int nC = static_cast<int>(k.size()); nut.resize(nC);
...
static void deviceEpsReactionRealizable(const DeviceMesh& dm, const DeviceBuffer<scalar>& eps, const DeviceBuffer<scalar>& k,
...
void deviceBoundField(const DeviceMesh& dm, DeviceBuffer<scalar>& x, scalar floor) {
    const int nC = dm.nCells;
    DeviceBuffer<scalar> cl(static_cast<std::size_t>(nC)); boundClampKernel<<<nBlocks(nC), TPB>>>(nC, floor, x.data(), cl.data());
    DeviceBuffer<scalar> fi; deviceInterpolate(dm, cl, fi);                       // linearInterpolate(max(field,floor))
...
void deviceWallEpsG0(const DeviceWallData& w, const DeviceBuffer<scalar>& k, const DeviceBuffer<scalar>& Ux,
    ...
    eps0.resize(nC); G0.resize(nC);   // zero on-device (memsetAsync) instead of a host-vector alloc + H2D every iter
...
// apply the eps near-wall setValues constraint, fold, BiCGStab (loose relTol), bound. Extracted verbatim
// from the duplicated eps/k blocks (identical kernel order, so bit-identical); reused by k-omega SST.
template <class Reaction>
static void deviceSolveScalarTransport(const DeviceMesh& dm, const DeviceBoundary& db, DeviceBuffer<scalar>& field,
                                       const char* fieldName,
                                       const DeviceBuffer<scalar>& D, const DeviceBuffer<scalar>& phiInt,
                                       const DeviceBuffer<scalar>& phiBnd, const DeviceBuffer<scalar>& divU,
                                       bool bounded, bool limited, bool linearUpwind, bool nonOrth, scalar twoByk, scalar relax, scalar tol,
                                       scalar relTolKE, int keCheckEvery, bool useGS, Reaction&& reaction,
                                       const DeviceWallData* wall = nullptr, const DeviceBuffer<scalar>* eps0 = nullptr,
                                       DeviceAMI* ami = nullptr, DeviceCyclic* cyc = nullptr) {
    const int nC = dm.nCells;
    DeviceBuffer<scalar> Df; deviceInterpolate(dm, D, Df);
    ...
    if (wall && eps0) {                                           // eps near-wall setValues constraint (k has none)
        ...
        if (ami && ami->n) deviceAmiZeroWallIfCoeff(*ami, wall->isWallCell);   // wall∩interface: don't perturb the fixed eps
...
void deviceKEpsilonCorrect(const DeviceMesh& dm, const DeviceWallData& wall, const DeviceBoundary& dbEps,
                           ...
                           scalar nu, scalar relaxEps, scalar relaxK, scalar tol, bool bounded,
                           bool limitedK, bool limitedEps, scalar twoBykK, scalar twoBykEps,
                           const KEpsilonCoeffs& co, scalar relTolKE, int keCheckEvery,
                           bool linearUpwindK, bool linearUpwindEps, bool nonOrth,
                           bool gsK, bool gsEps, DeviceAMI* ami, DeviceCyclic* cyc) {
    const int nC = dm.nCells;
    ...
    DeviceBuffer<scalar> G; deviceHadamard(G, nut, gByNu);
    ...
    DeviceBuffer<scalar> eps0, G0; deviceWallEpsG0(wall, k, Ux, Uy, Uz, nu, eps0, G0, co);
    ...
    DeviceBuffer<scalar> Deps(static_cast<std::size_t>(nC)); depsKernel<<<nBlocks(nC), TPB>>>(nC, nut.data(), co.sigmaEps, nu, Deps.data());
    ...
    DeviceBuffer<scalar> Dk(static_cast<std::size_t>(nC)); depsKernel<<<nBlocks(nC), TPB>>>(nC, nut.data(), co.sigmaK, nu, Dk.data());
...
// LIMITED, else on skewed cells the unlimited strain is huge and the SST nut limiter max(a1ω, b1·F2·√S2) wrongly
// fires (nut up to 30× low; verified cell-by-cell vs OF). Apply the per-component minmod limiter to the gradU TENSOR:
...
void deviceCellLimitGradU(const DeviceMesh& dm, const DeviceVectorBoundary& dbU,
    ...
    const int nC = dm.nCells; const DeviceBuffer<scalar>* U[3] = {&Ux, &Uy, &Uz};
    for (int j = 0; j < 3; ++j) {
        ...
        DeviceBuffer<scalar> ubv; deviceBCValue(dbU.comp[j], *U[j], ubv);
...
void deviceKOmegaSSTCorrect(const DeviceMesh& dm, const DeviceWallData& wall, const DeviceBoundary& dbOmega,
                            ...
                            scalar nu, scalar relaxOmega, scalar relaxK, scalar tol, bool bounded,
                            bool limitedK, bool limitedOmega, scalar twoBykK, scalar twoBykOmega,
                            const KOmegaSSTCoeffs& co, scalar relTolKE, int keCheckEvery,
                            bool linearUpwindK, bool linearUpwindOmega, bool nonOrth, scalar gradULimitK,
                            bool gsK, bool gsEps, DeviceAMI* ami, DeviceCyclic* cyc, const scalar* gammaIntEff) {
    const int nC = dm.nCells;
    // production (raw GbyNu0) + G = nut*GbyNu0, divU, S2 (shared gradU = OF tgradU = grad(U) scheme).
    DeviceBuffer<scalar> gradU; deviceGradU(dm, dbU, Ux, Uy, Uz, gradU, ami, cyc);   // interface-aware grad(U)
    ...
    DeviceBuffer<scalar> omega0, G0; deviceWallOmegaG0(wall, k, Ux, Uy, Uz, nu, omega0, G0, co);
    ...
    DeviceBuffer<scalar> kbv; deviceBCValue(dbK, k, kbv);
    DeviceBuffer<scalar> kgx, kgy, kgz; deviceGaussGrad(dm, k, kbv, kgx, kgy, kgz);
    DeviceBuffer<scalar> obv; deviceBCValue(dbOmega, omega, obv);
    DeviceBuffer<scalar> ogx, ogy, ogz; deviceGaussGrad(dm, omega, obv, ogx, ogy, ogz);
    DeviceBuffer<scalar> CD; deviceCDkOmega(kgx, kgy, kgz, ogx, ogy, ogz, omega, co.alphaOmega2, CD);
    DeviceBuffer<scalar> F1; deviceF1(k, omega, y, CD, nu, co, F1, gammaIntEff != nullptr);   // LM: F1=max(F1,F3) near-wall override
...
__global__ void lmReDiffKernel(int nC, const scalar* __restrict__ nut, scalar sigma, scalar nu, scalar* __restrict__ D) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC) D[c] = sigma*(nut[c] + nu);
}
// diag += V*sp ; source += V*su  (apply a precomputed semi-implicit reaction).
__global__ void lmAddReactionKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ sp,
                                    const scalar* __restrict__ su, scalar* __restrict__ diag, scalar* __restrict__ source) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    diag[c] += V[c] * sp[c]; source[c] += V[c] * su[c];
}
// per-cell strain helpers from the OF-convention gradU tensor (t[i*3+j] = dU_j/dx_i).
__device__ __forceinline__ void lmStrain(const scalar* t, scalar ux, scalar uy, scalar uz, scalar deltaU,
                                         scalar& S, scalar& Omega, scalar& Us, scalar& dUsds) {
    scalar symSq = 0.0, skSq = 0.0;
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) {
        const scalar sy = 0.5*(t[i*3+j]+t[j*3+i]), sk = 0.5*(t[i*3+j]-t[j*3+i]); symSq += sy*sy; skSq += sk*sk; }
    S = sqrt(2.0*symSq); Omega = sqrt(2.0*skSq);
    Us = fmax(sqrt(ux*ux+uy*uy+uz*uz), deltaU);
    const scalar Uv[3] = {ux, uy, uz};
    scalar num = 0.0; for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) num += Uv[i]*Uv[j]*t[i*3+j];
    dUsds = num/(Us*Us);
}
// Fthetat (OF kOmegaSSTLM::Fthetat), uses the lagged gammaInt; reused by both the ReThetat source and gammaSep.
__device__ __forceinline__ scalar lmFthetat(scalar S, scalar Omega, scalar Us, scalar nu, scalar y, scalar om,
...
__device__ __forceinline__ scalar lmReThetac(scalar R) {
    return (R <= 1870.0) ? R - 396.035e-2 + 120.656e-4*R - 868.230e-6*R*R + 696.506e-9*R*R*R - 174.105e-12*R*R*R*R
                         : R - 593.11 - 0.482*(R - 1870.0);
}
__device__ __forceinline__ scalar lmFlength(scalar R, scalar y, scalar om, scalar nu) {
    scalar Fl;
    ...
    const scalar fs = y*y*om/(200.0*nu); const scalar Fsub = exp(-(fs*fs));
...
__global__ void lmReThetatPrepKernel(int nC, const scalar* __restrict__ gradU, const scalar* __restrict__ Ux,
        const scalar* __restrict__ Uy, const scalar* __restrict__ Uz, const scalar* __restrict__ k,
        const scalar* __restrict__ om, const scalar* __restrict__ y, const scalar* __restrict__ ReThetat,
        const scalar* __restrict__ gammaInt, scalar nu, scalar cThetat, scalar ce2, scalar deltaU, scalar lambdaErr,
        int maxIter, scalar* __restrict__ Fth, scalar* __restrict__ sp, scalar* __restrict__ su) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    scalar t[9]; for (int q = 0; q < 9; ++q) t[q] = gradU[q*nC+c];
    scalar S, Omega, Us, dUsds; lmStrain(t, Ux[c], Uy[c], Uz[c], deltaU, S, Omega, Us, dUsds);
    ...
    scalar lambda = 0.0, thetat = 0.0; int iter = 0;
    for (;;) {
    ...
    sp[c] = Pthetat; su[c] = Pthetat*ReThetat0;
}
// gammaInt reaction prep: Pgamma, Egamma -> sp=ce1*Pgamma+ce2*Egamma, su=Pgamma+Egamma.
__global__ void lmGammaPrepKernel(int nC, const scalar* __restrict__ gradU, const scalar* __restrict__ Ux,
        const scalar* __restrict__ Uy, const scalar* __restrict__ Uz, const scalar* __restrict__ k,
        const scalar* __restrict__ om, const scalar* __restrict__ y, const scalar* __restrict__ ReThetat,
        const scalar* __restrict__ gammaInt, scalar nu, scalar ca1, scalar ca2, scalar ce1, scalar ce2, scalar deltaU,
        scalar* __restrict__ sp, scalar* __restrict__ su) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    scalar t[9]; for (int q = 0; q < 9; ++q) t[q] = gradU[q*nC+c];
    scalar S, Omega, Us, dUsds; lmStrain(t, Ux[c], Uy[c], Uz[c], deltaU, S, Omega, Us, dUsds);
    ...
    sp[c] = ce1*Pgamma + ce2*Egamma; su[c] = Pgamma + Egamma;
}
// gammaIntEff = max(gammaInt, gammaSep); gammaSep = min(2*max(Rev/(3.235*ReThetac)-1,0)*Freattach, 2)*Fthetat.
__global__ void lmGammaEffKernel(int nC, const scalar* __restrict__ gradU, const scalar* __restrict__ Ux,
        const scalar* __restrict__ Uy, const scalar* __restrict__ Uz, const scalar* __restrict__ k,
        const scalar* __restrict__ om, const scalar* __restrict__ y, const scalar* __restrict__ ReThetat,
        const scalar* __restrict__ gammaInt, const scalar* __restrict__ Fth, scalar nu, scalar deltaU,
...
void deviceKOmegaSSTLMCorrect(const DeviceMesh& dm, const DeviceVectorBoundary& dbU, const DeviceBoundary& dbReThetat,
                              ...
                              bool gsEps, DeviceAMI* ami, DeviceCyclic* cyc) {
    ...
    DeviceBuffer<scalar> DRe(nC); lmReDiffKernel<<<nBlocks(nC), TPB>>>(nC, nut.data(), lm.sigmaThetat, nu, DRe.data());  // sigmaThetat*(nut+nu)
    ...
    DeviceBuffer<scalar> DgI(nC); depsKernel<<<nBlocks(nC), TPB>>>(nC, nut.data(), 1.0, nu, DgI.data());   // nut/1 + nu
...
__global__ void saStildaKernel(int nC, const scalar* __restrict__ gradU, const scalar* __restrict__ nt,
                               const scalar* __restrict__ y, scalar nu, SpalartAllmarasCoeffs co, scalar* __restrict__ Stilda) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
...
__global__ void saFwKernel(int nC, const scalar* __restrict__ nt, const scalar* __restrict__ Stilda,
                           const scalar* __restrict__ y, SpalartAllmarasCoeffs co, scalar* __restrict__ fw) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    const scalar kd2 = fmax(co.kappa*y[c]*co.kappa*y[c], 1e-300);
...
__global__ void saReactionKernel(int nC, const scalar* __restrict__ V, const scalar* __restrict__ nt,
                                 const scalar* __restrict__ Stilda, const scalar* __restrict__ fw,
                                 const scalar* __restrict__ y, const scalar* __restrict__ gradNt2,
                                 SpalartAllmarasCoeffs co, scalar* __restrict__ diag, scalar* __restrict__ src) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
...
__global__ void saDEffKernel(int nC, const scalar* __restrict__ nt, scalar nu, scalar sigma, scalar* __restrict__ D) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC) D[c] = (nt[c] + nu) / sigma;
}
__global__ void saNutKernel(int nC, const scalar* __restrict__ nt, scalar nu, scalar Cv1, scalar* __restrict__ nut) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c >= nC) return;
    const scalar chi = nt[c]/nu, chi3 = chi*chi*chi, Cv13 = Cv1*Cv1*Cv1;
    nut[c] = nt[c] * (chi3 / (chi3 + Cv13));                                                  // nut = nuTilda*fv1
}
__global__ void saMagSqrKernel(int nC, const scalar* __restrict__ gx, const scalar* __restrict__ gy,
                               const scalar* __restrict__ gz, scalar* __restrict__ out) {
    const int c = blockIdx.x * blockDim.x + threadIdx.x; if (c < nC) out[c] = gx[c]*gx[c]+gy[c]*gy[c]+gz[c]*gz[c];
}
...
__global__ void spaldingNutKernel(int n, const label* __restrict__ fc, const label* __restrict__ isWall,
                                  const scalar* __restrict__ y, const scalar* __restrict__ dc,
                                  const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz,
                                  const scalar* __restrict__ nutCell, scalar nu, scalar kappa, scalar E, scalar* __restrict__ nutBnd) {
...
void deviceBoundaryNutSpalding(const DeviceVectorBoundary& dbU, const DeviceBuffer<label>& isWall,
    ...
    if (static_cast<int>(nutBnd.size()) != n) {                      // first call: zero seed (warm-starts thereafter)
        nutBnd.resize(n); cudaCheck(cudaMemsetAsync(nutBnd.data(), 0, n*sizeof(scalar), cudaStreamPerThread), "spalding init");
    }
...
void deviceSpalartAllmarasCorrect(const DeviceMesh& dm, const DeviceVectorBoundary& dbU, const DeviceBoundary& dbNuTilda,
                                  ...
                                  const DeviceBuffer<scalar>& phiInt, const DeviceBuffer<scalar>& phiBnd,
                                  scalar nu, scalar relax, scalar tol, bool bounded, bool limited, scalar twoByk,
                                  const SpalartAllmarasCoeffs& co, scalar relTol, int checkEvery, bool linearUpwind, bool nonOrth,
                                  bool gsK, DeviceAMI* ami, DeviceCyclic* cyc) {
    const int nC = dm.nCells;
    DeviceBuffer<scalar> gradU; deviceGradU(dm, dbU, Ux, Uy, Uz, gradU, ami, cyc);   // interface-aware grad(U) for vorticity/production
    ...
    DeviceBuffer<scalar> nbv; deviceBCValue(dbNuTilda, nuTilda, nbv);                          // |grad nuTilda|^2
    DeviceBuffer<scalar> gnx, gny, gnz; deviceGaussGrad(dm, nuTilda, nbv, gnx, gny, gnz);
    ...
    DeviceBuffer<scalar> divU; deviceDiv(dm, phiInt, phiBnd, divU);                            // for the bounded term
    if (ami && ami->n) deviceAmiAddDiv(*ami, dm.V, divU);
    if (cyc && cyc->n) deviceCyclicAddDiv(*cyc, dm.V, divU);
...
void deviceNutSA(const DeviceBuffer<scalar>& nuTilda, scalar nu, scalar Cv1, DeviceBuffer<scalar>& nut) {
    const int nC = static_cast<int>(nuTilda.size()); nut.resize(nC);
