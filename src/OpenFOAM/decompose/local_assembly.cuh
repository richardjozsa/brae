...
inline std::vector<std::vector<scalar>> computeProcDeltaCoeffs(
    const LocalMesh& lm,
    const FvGeometry& lg,
...
template <typename T>
inline DistributedMatrix momentumDistributed(
    const FvMatrix<T>& Mlocal,
    const LocalMesh& lm,
    const FvGeometry& lg,
    const std::vector<FvPatch>& lpatches,
    const std::vector<scalar>& localPhi,
    const std::vector<scalar>& localNuEffF,
    const std::vector<std::vector<scalar>>& procDelta)
{
    ...
    L.upper = Mlocal.upper;
    L.lower = Mlocal.lower;
    std::size_t pj = 0;
    for (const FvPatch& p : lpatches)
    {
...
inline std::vector<std::vector<scalar>> computeProcWeights(
    const LocalMesh& lm,
    const FvGeometry& lg,
    const std::vector<FvPatch>& lpatches)
{
...
inline DistributedMatrix assembleLocalLaplacianF(
    const LocalMesh& lm,
    const FvGeometry& lg,
    const std::vector<FvPatch>& lpatches,
    const std::vector<std::vector<scalar>>& procDelta,
...
inline DistributedMatrix assembleLocalLaplacian(
    const LocalMesh& lm,
    const FvGeometry& lg,
    const std::vector<FvPatch>& lpatches,
    const std::vector<std::vector<scalar>>& procDelta,
...
inline DistributedMatrix assembleLocalConvection(
    const LocalMesh& lm,
    const std::vector<FvPatch>& lpatches,
    const std::vector<scalar>& localPhi)
{
