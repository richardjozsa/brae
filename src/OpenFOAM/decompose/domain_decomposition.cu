    ...
    int myPart)
{
    ...
    for (auto& kv : byNbr)
    {
        ...
        faceCells.reserve(v.size());
        nbrG.reserve(v.size());
        gFaces.reserve(v.size());
        ownerSide.reserve(v.size());
        for (const std::array<label, 4>& t : v)
        {
            gFaces.push_back(t[0]);
            faceCells.push_back(t[1]);
            nbrG.push_back(t[2]);
            ownerSide.push_back(static_cast<char>(t[3]));
        }
