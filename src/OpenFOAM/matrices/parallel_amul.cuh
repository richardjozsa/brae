...
inline DistributedMatrix distribute(const FvScalarMatrix& global, const DomainDecomposition& decomp) {
    ...
    L.upper.resize(lfg.size()); L.lower.resize(lfg.size());
    for (std::size_t f = 0; f < lfg.size(); ++f) { L.upper[f] = global.upper[lfg[f]]; L.lower[f] = global.lower[lfg[f]]; }
...
inline void foldBoundary(DistributedMatrix& L, const FvScalarMatrix& global,
                         const std::vector<FvPatch>& gPatches, const std::vector<label>& addr,
                         const std::vector<label>& cellToPart, int myPart) {
...
inline void addEqualDist(DistributedMatrix& L, const DistributedMatrix& R, scalar s) {
    ...
    for (std::size_t f = 0; f < L.upper.size(); ++f) { L.upper[f] += s * R.upper[f]; L.lower[f] += s * R.lower[f]; }
...
inline std::vector<scalar> parallelAmul(const DistributedMatrix& L, const std::vector<scalar>& psi,
