...
inline SolverPerformance parallelPBiCGStab(
    ...
    int minIter = 0)
{
    ...
    {
        Ax = parallelAmul(Lf, x, interfaces);
    };
    auto gSum = [&](scalar s)
    {
    ...
    for (label c = 0; c < nC; ++c)
    {
        ...
        nf += std::fabs(yA[c] - t) + std::fabs(b[c] - t);
    }
    ...
    {
        return (fr < tol) || (relTol > 0.0 && fr < relTol * perf.initialResidual);
    };
