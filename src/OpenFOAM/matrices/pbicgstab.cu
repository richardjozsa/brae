...
SolverPerformance pbicgstab(const FvScalarMatrix& M,
    ...
    auto sumSqr  = [&](const std::vector<scalar>& a) { scalar s = 0; for (label i = 0; i < nC; ++i) s += a[i] * a[i]; return s; };
    ...
    for (label c = 0; c < nC; ++c) { const scalar t = sumA[c] * xRef; normFactor += std::fabs(yA[c] - t) + std::fabs(b[c] - t); }
    normFactor += 1e-20;
