...
inline CyclicMatrix assembleCyclicLaplacian(const PrimitiveMesh& m, const FvGeometry& g,
                                            const std::vector<FvPatch>& fvp,
                                            const std::vector<CyclicInterface>& cyclics, scalar gamma) {
    const label nC = m.nCells(), nIf = m.nInternalFaces();
    const std::vector<label>& own = m.owner(); const std::vector<label>& nei = m.neighbour();
    const std::vector<scalar>& dc = g.deltaCoeffs(); const std::vector<scalar>& magSf = g.magSf();

    CyclicMatrix M; M.diag.assign(nC, 0.0); M.upper.resize(nIf); M.lower.resize(nIf);
...
inline std::vector<scalar> cyclicAmul(const CyclicMatrix& M, const PrimitiveMesh& m,
    ...
    for (label f = 0; f < nIf; ++f) { y[own[f]] += M.upper[f] * psi[nei[f]]; y[nei[f]] += M.lower[f] * psi[own[f]]; }
    for (std::size_t ci = 0; ci < cyclics.size(); ++ci) {
        const CyclicInterface& c = cyclics[ci];
        for (std::size_t i = 0; i < c.faceCells.size(); ++i)
...
inline int cyclicPCG(const CyclicMatrix& M, const PrimitiveMesh& m, const std::vector<CyclicInterface>& cyclics,
    ...
    auto dot = [&](const std::vector<scalar>& a, const std::vector<scalar>& c) {
        scalar s = 0; for (label i = 0; i < nC; ++i) s += a[i] * c[i]; return s; };
    std::vector<scalar> r(nC), z(nC), p(nC), Ap;
    const std::vector<scalar> Ax = cyclicAmul(M, m, cyclics, x);
    for (label i = 0; i < nC; ++i) { r[i] = b[i] - Ax[i]; z[i] = r[i] / M.diag[i]; }
    p = z; scalar rz = dot(r, z); int it = 0;
    for (; it < maxIter; ++it) {
        Ap = cyclicAmul(M, m, cyclics, p);
        const scalar pAp = dot(p, Ap); if (std::fabs(pAp) < 1e-300) break;
        const scalar alpha = rz / pAp;
        for (label i = 0; i < nC; ++i) { x[i] += alpha * p[i]; r[i] -= alpha * Ap[i]; }
        if (std::sqrt(dot(r, r)) < tol) { ++it; break; }
        for (label i = 0; i < nC; ++i) z[i] = r[i] / M.diag[i];
        const scalar rz2 = dot(r, z), beta = rz2 / rz; rz = rz2;
