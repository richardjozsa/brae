...
template<class T> void wvec(std::FILE* f, const std::vector<T>& v) {
    std::size_t n = v.size(); std::fwrite(&n, sizeof(n), 1, f); if (n) std::fwrite(v.data(), sizeof(T), n, f); }
template<class T> bool rvec(std::FILE* f, std::vector<T>& v) {
    std::size_t n; if (std::fread(&n, sizeof(n), 1, f) != 1) return false; v.resize(n);
    return n == 0 || std::fread(v.data(), sizeof(T), n, f) == n; }
void wstr(std::FILE* f, const std::string& s) {
    std::size_t n = s.size(); std::fwrite(&n, sizeof(n), 1, f); if (n) std::fwrite(s.data(), 1, n, f); }
bool rstr(std::FILE* f, std::string& s) {
    std::size_t n; if (std::fread(&n, sizeof(n), 1, f) != 1) return false; s.resize(n);
...
void PrimitiveMesh::writeBinary(const std::string& path) const {
    ...
    unsigned magic = BRAE_MESH_CACHE_MAGIC; std::fwrite(&magic, sizeof(magic), 1, f);
    std::fwrite(&nCells_, sizeof(nCells_), 1, f);
    wvec(f, points_); wvec(f, faceVerts_); wvec(f, faceOffsets_); wvec(f, owner_); wvec(f, neighbour_);
    std::size_t np = patches_.size(); std::fwrite(&np, sizeof(np), 1, f);
    for (const auto& p : patches_) {
        wstr(f, p.name); wstr(f, p.type);
        std::fwrite(&p.start, sizeof(p.start), 1, f); std::fwrite(&p.size, sizeof(p.size), 1, f);
        wstr(f, p.neighbourPatch);
        std::size_t ng = p.inGroups.size(); std::fwrite(&ng, sizeof(ng), 1, f); for (const auto& g : p.inGroups) wstr(f, g);
        wstr(f, p.transform);
        std::fwrite(&p.rotationAxis, sizeof(p.rotationAxis), 1, f); std::fwrite(&p.rotationCentre, sizeof(p.rotationCentre), 1, f);
    }
    std::fclose(f);
}
...
bool PrimitiveMesh::loadBinary(const std::string& path) {
    ...
    unsigned magic = 0; bool ok = std::fread(&magic, sizeof(magic), 1, f) == 1 && magic == BRAE_MESH_CACHE_MAGIC;
    ...
    std::size_t np = 0; ok = ok && std::fread(&np, sizeof(np), 1, f) == 1;
    if (ok) { patches_.resize(np);
        for (auto& p : patches_) {
            ...
            std::size_t ng = 0; ok = ok && std::fread(&ng, sizeof(ng), 1, f) == 1;
            if (ok) { p.inGroups.resize(ng); for (auto& g : p.inGroups) ok = ok && rstr(f, g); }
...
void PrimitiveMesh::readPoints(const std::string& dir) {
    ...
    if (foamFormat(path) == "binary") { points_ = readBinaryPoints(path); return; }
...
void PrimitiveMesh::readFaces(const std::string& dir) {
    ...
    if (foamFormat(path) == "binary") { readBinaryCompactFaces(path, faceOffsets_, faceVerts_); return; }
