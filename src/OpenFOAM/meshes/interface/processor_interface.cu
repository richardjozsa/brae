...
ProcessorInterface::ProcessorInterface(int myPart, int nbrPart,
                                       std::vector<label> faceCells, int tag)
    : myPart_(myPart),
      nbrPart_(nbrPart),
      tag_(tag),
...
void ProcessorInterface::interfaceInternalField(const scalar* psi,
