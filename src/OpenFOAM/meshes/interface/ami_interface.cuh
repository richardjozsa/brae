...
inline scalar signedArea(const std::vector<vec2>& p) {
    scalar a = 0; const int n = (int)p.size();
    for (int i = 0; i < n; ++i) { const vec2 &u = p[i], &v = p[(i+1)%n]; a += u.x*v.y - v.x*u.y; }
    return 0.5*a;
}
...
inline std::vector<vec2> clipPoly(const std::vector<vec2>& subject, const std::vector<vec2>& clip) {
    std::vector<vec2> out = subject; const int nc = (int)clip.size();
    for (int e = 0; e < nc && !out.empty(); ++e) {
        const vec2 A = clip[e], B = clip[(e+1)%nc];                 // clip edge A->B (inside = left, CCW)
        const std::vector<vec2> in = out; out.clear();
...
inline std::vector<AMIInterface> buildAMIInterfaces(
    ...
    auto facePoly = [&](label gf) {
        std::vector<vector> P; for (label k = fo[gf]; k < fo[gf+1]; ++k) P.push_back(pts[fv[k]]); return P; };

    std::vector<AMIInterface> out;
    for (label pi = 0; pi < (label)fvp.size(); ++pi) {
        if (fvp[pi].type != "cyclicAMI") continue;
        AMIInterface ai; ai.patch = pi;
        ...
        const FvPatch& S = fvp[pi]; const FvPatch& T = fvp[ai.nbrPatch];

        // transform (nbr/target -> own/source); same convention as cyclic
        vector axis{0,0,1}; vector ctr{0,0,0};
        if (!ai.translational) {
            axis = pinfo[pi].rotationAxis / mag(pinfo[pi].rotationAxis); ctr = pinfo[pi].rotationCentre;
        ...
        e1 = e1 / std::fmax(mag(e1), 1e-300); vector e2 = cross(n, e1);
        const vector orig = g.Cf()[S.start];
        auto proj = [&](const vector& p) -> vec2 { const vector d = p - orig; return { dot(d,e1), dot(d,e2) }; };

        // project source faces + map+project target faces
        std::vector<std::vector<vec2>> srcP(S.size), tgtP(T.size);
        for (label i = 0; i < S.size; ++i) { for (const vector& v : facePoly(S.start+i)) srcP[i].push_back(proj(v)); orientCCW(srcP[i]); }
        for (label j = 0; j < T.size; ++j) { for (const vector& v : facePoly(T.start+j)) tgtP[j].push_back(proj(mapTtoS(v))); orientCCW(tgtP[j]); }
        ...
        ai.ownCell = S.faceCells; ai.srcOffset.assign(S.size + 1, 0);
        ...
        for (label i = 0; i < S.size; ++i) {
            ...
            scalar s = 0; for (auto& e : stencil[i]) s += e.second;
        ...
        auto rotDelta = [&](const vector& d) -> vector { return ai.translational ? d : dot(d, transpose(ai.forwardT)); };
        for (label i = 0; i < S.size; ++i) {
            const vector Sfi = g.Sf()[S.start+i]; const scalar msf = g.magSf()[S.start+i]; const vector nf = Sfi/msf;
            const vector patchD = g.Cf()[S.start+i] - g.C()[S.faceCells[i]];
            vector nbrDint{0,0,0}; scalar dni = 0;
            for (const auto& e : stencil[i]) {
                ...
                nbrDint += e.second * rotDelta(nbrD);                             // Σ w · transform(nbrD)  (for delta)
                dni     += e.second * dot(nfn, nbrD);                             // Σ w · (nfn & nbrD)  (OF makeWeights)
            }
            const vector delta = patchD - nbrDint;
            const scalar di = dot(nf, patchD);
            ai.magSf.push_back(msf); ai.Sf.push_back(Sfi);
            ai.weights.push_back(std::fabs(di + dni) > 1e-300 ? dni/(di + dni) : 0.5);
            const scalar dc = 1.0 / std::fmax(dot(nf, delta), 0.05 * mag(delta));
            ai.deltaCoeffs.push_back(dc); ai.corrVec.push_back(nf - delta * dc);
            ai.dOwn.push_back(patchD);   // own-side linearUpwind reconstruction delta (Cf - C_own)
            for (const auto& e : stencil[i]) { const label j = e.first;
                ai.nbrCell.push_back(T.faceCells[j]); ai.weight.push_back(e.second);
