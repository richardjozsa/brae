...
inline std::regex compileFoamRegex(const std::string& pat) {
    ...
    std::string p = pat;
    if (p.size() >= 4 && p.compare(0, 4, "(?i)") == 0) { flags |= std::regex::icase; p.erase(0, 4); }
...
struct FoamDict {
    ...
    const FoamDict* subDict(const std::string& key) const {
        for (const auto& s : subs) if (s.first == key) return &s.second;          // literal wins
        const FoamDict* hit = nullptr;
        for (const auto& s : subs) {                                              // else last regex match
            if (s.first == key) continue;
            try { if (std::regex_match(key, compileFoamRegex(s.first))) hit = &s.second; } catch (...) {}
        }
        return hit;
    }
    ...
    const std::vector<std::string>* find(const std::string& name) const {
        for (const auto& l : leaves) if (l.first == name) return &l.second;          // literal wins
        const std::vector<std::string>* hit = nullptr;
        for (const auto& l : leaves) {                                                // else last regex match
            if (l.first == name) continue;
            try { if (std::regex_match(name, compileFoamRegex(l.first))) hit = &l.second; } catch (...) {}
        }
        return hit;
    }
    ...
    scalar scalarOr(const std::string& name, scalar def) const {
        const auto* v = find(name); return (v && !v->empty()) ? std::stod(v->back()) : def;
    }
    int intOr(const std::string& name, int def) const {
        const auto* v = find(name); return (v && !v->empty()) ? std::stoi(v->back()) : def;
    }
    std::string wordOr(const std::string& name, const std::string& def) const {
        const auto* v = find(name); return (v && !v->empty()) ? v->front() : def;
    }
    ...
    std::vector<scalar> scalarListOr(const std::string& name, const std::vector<scalar>& def) const {
        const auto* t = find(name); if (!t) return def;
        std::string joined; for (const auto& s : *t) joined += " " + s;
    ...
    std::vector<std::string> wordListOr(const std::string& name, const std::vector<std::string>& def) const {
        ...
        for (const auto& s : *t) { std::string w; for (char ch : s) if (ch != '(' && ch != ')') w += ch; if (!w.empty()) out.push_back(w); }
...
inline FoamDict parseDictBody(TokenStream& ts, bool top) {
    FoamDict d;
    while (!ts.eof()) {
        if (ts.peek() == "}") { if (!top) ts.next(); break; }
        const std::string key = ts.next();
        if (ts.eof()) break;
        if (ts.peek() == "{") { ts.next(); d.subs.emplace_back(key, parseDictBody(ts, false)); }
        else {
...
inline FoamDict readDict(const std::string& path) { TokenStream ts(path); return parseDictBody(ts, true); }
...
inline std::string expandDictVariables(const std::string& rawIn) {
    // 1. strip comments
    std::string raw; raw.reserve(rawIn.size());
    for (std::size_t i = 0; i < rawIn.size(); ) {
        if (rawIn[i] == '/' && i + 1 < rawIn.size() && rawIn[i + 1] == '/') { while (i < rawIn.size() && rawIn[i] != '\n') ++i; }
        else if (rawIn[i] == '/' && i + 1 < rawIn.size() && rawIn[i + 1] == '*') {
            i += 2; while (i + 1 < rawIn.size() && !(rawIn[i] == '*' && rawIn[i + 1] == '/')) ++i; i += 2;
        } else raw += rawIn[i++];
    }
    // 2. tokenize with { } ; as separate tokens (parens stay inside their token, e.g. div(phi,U))
    std::string padded; padded.reserve(raw.size() * 2);
    for (char c : raw) { if (c == '{' || c == '}' || c == ';') { padded += ' '; padded += c; padded += ' '; } else padded += c; }
    std::vector<std::string> toks; { std::istringstream is(padded); std::string t; while (is >> t) toks.push_back(t); }
    ...
    std::map<std::string, std::string> vars; bool atKey = true;
    for (std::size_t i = 0; i < toks.size(); ) {
        const std::string& tk = toks[i];
        if (tk == "{" || tk == "}" || tk == ";") { atKey = true; ++i; continue; }
        if (!atKey) { ++i; continue; }
        if (i + 1 < toks.size() && toks[i + 1] == "{") {   // SUBDICT (e.g. intakeType1 { ... } or divSchemes { ... }):
            std::size_t j = i + 2; int depth = 1; std::string val;   // capture its CONTENT so $intakeType1 -> entries (OF dict-merge)
            ...
            i += 2; atKey = true; continue;
        }
        std::string val; std::size_t j = i + 1;
    ...
    for (int pass = 0; pass < 5; ++pass) {
        std::string out; out.reserve(text.size()); bool changed = false;
        for (std::size_t i = 0; i < text.size(); ) {
            if (text[i] == '$') {
                std::size_t j = i + 1; bool brace = false; if (j < text.size() && text[j] == '{') { brace = true; ++j; }
                ...
                if (!name.empty() && it != vars.end()) { out += it->second; i = j; changed = true; continue; }
            }
            out += text[i++];
        }
        text = std::move(out); if (!changed) break;
    }
    return text;
}
inline std::string readFileExpanded(const std::string& path) {
    std::ifstream f(path); std::stringstream ss; ss << f.rdbuf(); return expandDictVariables(ss.str());
}
