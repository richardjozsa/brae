...
std::vector<char> gzSlurp(const std::string& base, std::size_t maxBytes = 0) {
    ...
    { std::ifstream a(base, std::ios::binary);
      if (!a.good()) { std::ifstream g(base + ".gz", std::ios::binary); if (g.good()) path = base + ".gz"; } }
    gzFile f = gzopen(path.c_str(), "rb");
    if (!f) throw std::runtime_error("brae: cannot open " + base + " (or " + base + ".gz)");
    std::vector<char> out; char buf[1 << 16]; int n;
...
std::string dirOf(const std::string& p) { const auto s = p.find_last_of('/'); return s == std::string::npos ? "." : p.substr(0, s); }
...
std::string stripFoamHeader(const std::string& s) {
    const auto f = s.find("FoamFile"); if (f == std::string::npos) return s;
    const auto b = s.find('{', f);     if (b == std::string::npos) return s;
    int depth = 0; std::size_t i = b;
    for (; i < s.size(); ++i) { if (s[i] == '{') ++depth; else if (s[i] == '}') { if (--depth == 0) { ++i; break; } } }
...
std::string resolveEtc(const std::string& rel) {
    ...
    for (const auto& r : roots) { std::ifstream t(r + "/" + rel); if (t.good()) return r + "/" + rel; }
    return "";
}
...
bool isSteadyForceFO(const std::string& name) {
    std::string l; l.reserve(name.size());
...
std::string expandEnvVars(const std::string& s) {
    ...
    for (std::size_t i = 0; i < s.size(); ) {
        if (s[i] == '$') {
            ...
            if (j < s.size() && s[j] == '{') { ++j; while (j < s.size() && s[j] != '}') name += s[j++]; if (j < s.size()) ++j; }
            else { while (j < s.size() && (std::isalnum(static_cast<unsigned char>(s[j])) || s[j] == '_')) name += s[j++]; }
                    ...
                    if (e != std::string::npos) {
                        ...
                        if (f.good()) { std::ostringstream ss; ss << f.rdbuf();
                            ...
                            if (bare && isSteadyForceFO(rel)) {
                                const std::string fp = resolveFunc(rel); std::ifstream ff(fp);
                                if (ff.good()) { std::ostringstream ss; ss << ff.rdbuf();
...
std::size_t readAsciiCount(const std::vector<char>& b, std::size_t& i) {
    ...
    std::size_t n = 0;
    bool any = false;
    while (i < b.size() && b[i] >= '0' && b[i] <= '9') { n = n * 10 + (b[i] - '0'); ++i; any = true; }
