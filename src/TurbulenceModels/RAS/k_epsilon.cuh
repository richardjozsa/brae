...
inline std::vector<scalar> GbyNu(const std::vector<tensor>& gradU) {
    ...
    for (std::size_t c = 0; c < gradU.size(); ++c) {
        ...
        dts.xx -= t23; dts.yy -= t23; dts.zz -= t23;   // devTwoSymm
        g[c] = doubleDot(t, dts);
    }
    return g;
}

inline std::vector<scalar> nut(const std::vector<scalar>& k, const std::vector<scalar>& epsilon, const KEpsilonCoeffs& co = {}) {
    std::vector<scalar> nu(k.size());
    for (std::size_t c = 0; c < k.size(); ++c) nu[c] = co.Cmu * k[c] * k[c] / epsilon[c];
    return nu;
}

inline std::vector<scalar> production(const std::vector<scalar>& nutf, const std::vector<scalar>& gByNu) {
    std::vector<scalar> G(nutf.size());
    for (std::size_t c = 0; c < nutf.size(); ++c) G[c] = nutf[c] * gByNu[c];
    return G;
}

inline std::vector<scalar> DkEff(const std::vector<scalar>& nutf, scalar nu, const KEpsilonCoeffs& co = {}) {
    std::vector<scalar> d(nutf.size());
    for (std::size_t c = 0; c < nutf.size(); ++c) d[c] = nutf[c] / co.sigmaK + nu;
    return d;
}
inline std::vector<scalar> DepsilonEff(const std::vector<scalar>& nutf, scalar nu, const KEpsilonCoeffs& co = {}) {
    std::vector<scalar> d(nutf.size());
    for (std::size_t c = 0; c < nutf.size(); ++c) d[c] = nutf[c] / co.sigmaEps + nu;
    return d;
}

// kEqn = div(phi,k) - laplacian(DkEff,k) + Sp(eps/k, k) == G.
inline FvScalarMatrix kEqn(const GeometricField<scalar>& k, const std::vector<scalar>& eps,
                           const std::vector<scalar>& G, const std::vector<scalar>& nutf,
                           const SurfaceScalarField& phi, scalar nu,
...
inline FvScalarMatrix epsilonEqn(const GeometricField<scalar>& eps, const std::vector<scalar>& k,
                                 const std::vector<scalar>& GbyNuF, const std::vector<scalar>& nutf,
                                 const SurfaceScalarField& phi, scalar nu,
                                 const PrimitiveMesh& m, const FvGeometry& g, const std::vector<FvPatch>& patches,
                                 const KEpsilonCoeffs& co = {}) {
...
inline std::vector<scalar> correctNut(const GeometricField<scalar>& k, const GeometricField<scalar>& eps,
                                      scalar /*nu*/, const KEpsilonCoeffs& co = {}) {
...
inline void correct(const GeometricField<vector>& U, GeometricField<scalar>& k, GeometricField<scalar>& eps,
                    GeometricField<scalar>& nutField, const SurfaceScalarField& phi, scalar nu,
                    const PrimitiveMesh& m, const FvGeometry& g, const std::vector<FvPatch>& patches,
                    scalar relaxEps, scalar relaxK, scalar tol, scalar relTol, int maxIter,
                    const KEpsilonCoeffs& co = {}) {
    ...
    for (label c = 0; c < nC; ++c) if (nw[c] > 0) {
        ...
        wallCells.push_back(c); epsVals.push_back(eps0[c]);
    }
