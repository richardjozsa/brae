...
inline void relaxScalar(DistributedMatrix& L, FvScalarMatrix& Mloc, const std::vector<scalar>& psi,
                        const Partition& P, scalar alpha) {
    if (alpha <= 0.0) return;
    const std::vector<FvPatch>& lp = P.lp; const label lnC = P.nCells(), nIf = P.Lm.mesh.nInternalFaces();
    ...
    std::size_t pj = 0;
    for (const FvPatch& p : lp) { if (p.type != "processor") continue; for (label i = 0; i < p.size; ++i) sumOff[p.faceCells[i]] += std::fabs(L.interfaceCoeffs[pj][i]); ++pj; }
    for (std::size_t pi = 0; pi < lp.size(); ++pi) for (label i = 0; i < lp[pi].size; ++i) L.diag[lp[pi].faceCells[i]] += std::fabs(Mloc.internalCoeffs[pi][i]);
    for (label c = 0; c < lnC; ++c) L.diag[c] = std::fmax(std::fabs(L.diag[c]), sumOff[c]) / alpha;
    for (std::size_t pi = 0; pi < lp.size(); ++pi) for (label i = 0; i < lp[pi].size; ++i) L.diag[lp[pi].faceCells[i]] -= Mloc.internalCoeffs[pi][i];
    for (label c = 0; c < lnC; ++c) Mloc.source[c] += (L.diag[c] - D0[c]) * psi[c];
}
...
inline void setValuesDist(DistributedMatrix& L, FvScalarMatrix& Mloc, const Partition& P,
                          const std::vector<label>& cells, const std::vector<scalar>& vals) {
    const PrimitiveMesh& m = P.Lm.mesh; const std::vector<FvPatch>& lp = P.lp;
    const label nIf = m.nInternalFaces();
    std::vector<std::vector<std::pair<label,bool>>> adj(m.nCells());
    for (label f = 0; f < nIf; ++f) { adj[L.lowerAddr[f]].push_back({f,true}); adj[L.upperAddr[f]].push_back({f,false}); }
    std::vector<std::vector<std::pair<label,label>>> badj(m.nCells()), iadj(m.nCells());
    { std::size_t pj = 0; for (std::size_t pi = 0; pi < lp.size(); ++pi) { const bool proc = lp[pi].type == "processor"; for (label i = 0; i < lp[pi].size; ++i) { if (proc) iadj[lp[pi].faceCells[i]].push_back({(label)pj, i}); else badj[lp[pi].faceCells[i]].push_back({(label)pi, i}); } if (proc) ++pj; } }
    for (std::size_t i = 0; i < cells.size(); ++i) {
        const label c = cells[i]; const scalar v = vals[i];
        for (const auto& fe : adj[c]) { const label f = fe.first; if (fe.second) Mloc.source[L.upperAddr[f]] -= L.lower[f]*v; else Mloc.source[L.lowerAddr[f]] -= L.upper[f]*v; L.upper[f] = 0.0; L.lower[f] = 0.0; }
        for (const auto& be : badj[c]) { Mloc.internalCoeffs[be.first][be.second] = 0.0; Mloc.boundaryCoeffs[be.first][be.second] = 0.0; }
        for (const auto& ie : iadj[c]) L.interfaceCoeffs[ie.first][ie.second] = 0.0;
    }
}
...
inline SolverPerformance solveScalarEqn(const Partition& P, FvScalarMatrix& Mloc, std::vector<scalar>& psi,
                           const std::vector<scalar>& outPhi, const std::vector<scalar>& gammaF,
                           std::vector<ProcessorInterface>& ifs, scalar relax, scalar tol,
                           const std::vector<label>& conCells, const std::vector<scalar>& conVals) {
    const std::vector<FvPatch>& lp = P.lp; const label lnC = P.nCells();
    DistributedMatrix L = momentumDistributed(Mloc, P.Lm, P.lg, P.lp, outPhi, gammaF, P.procDelta);
    relaxScalar(L, Mloc, psi, P, relax);
    for (std::size_t i = 0; i < conCells.size(); ++i) psi[conCells[i]] = conVals[i];
    if (!conCells.empty()) setValuesDist(L, Mloc, P, conCells, conVals);
    L.diagC = L.diag; L.b = Mloc.source;
    for (std::size_t pi = 0; pi < lp.size(); ++pi) for (label i = 0; i < lp[pi].size; ++i) { L.diagC[lp[pi].faceCells[i]] += Mloc.internalCoeffs[pi][i]; L.b[lp[pi].faceCells[i]] += Mloc.boundaryCoeffs[pi][i]; }
    for (std::size_t i = 0; i < conCells.size(); ++i) { L.diagC[conCells[i]] = L.diag[conCells[i]]; L.b[conCells[i]] = conVals[i] * L.diag[conCells[i]]; }
...
inline std::vector<scalar> faceGamma(const Partition& P, const std::vector<scalar>& gammaCell) {
    ...
    for (label f = 0; f < m.nInternalFaces(); ++f) g[f] = gf.internal[f];
    for (std::size_t pi = 0; pi < lp.size(); ++pi) if (lp[pi].type == "processor") for (label i = 0; i < lp[pi].size; ++i) g[lp[pi].start + i] = gfld.boundary[pi]->value()[i];
    return g;
}

// Parallel kEpsilon::correct(). U/k/eps/nut are distributed fields; phi the maintained flux.
inline TurbResidual parallelCorrect(const Partition& P, const GeometricField<vector>& U, GeometricField<scalar>& k,
                            GeometricField<scalar>& eps, GeometricField<scalar>& nutField,
                            const SurfaceScalarField& phi, scalar nu, scalar relaxEps, scalar relaxK, scalar tol,
                            bool bounded = false, const KEpsilonCoeffs& co = {}) {
    const PrimitiveMesh& m = P.Lm.mesh; const FvGeometry& g = P.lg; const std::vector<FvPatch>& patches = P.lp;
    ...
    for (std::size_t pi = 0; pi < patches.size(); ++pi) if (patches[pi].type == "wall") for (label i = 0; i < patches[pi].size; ++i) ++nw[patches[pi].faceCells[i]];
    ...
    for (std::size_t pi = 0; pi < patches.size(); ++pi) {
        ...
        const FvPatch& wp = patches[pi]; const std::vector<scalar>& y = yWall[pi];
        ...
        for (label i = 0; i < wp.size; ++i) { const label c = wp.faceCells[i]; const scalar w = 1.0/nw[c], kc = k.internal[c];
    ...
    std::vector<label> wallCells; std::vector<scalar> epsVals;
    for (label c = 0; c < nC; ++c) if (nw[c] > 0) { G[c] = G0[c]; eps.internal[c] = eps0[c]; wallCells.push_back(c); epsVals.push_back(eps0[c]); }

    // epsilon equation.
    std::vector<scalar> DepsCell(nC); for (label c = 0; c < nC; ++c) DepsCell[c] = nutF[c]/co.sigmaEps + nu;
    ...
    // k equation.
    std::vector<scalar> DkCell(nC); for (label c = 0; c < nC; ++c) DkCell[c] = nutF[c]/co.sigmaK + nu;
    ...
    const std::vector<label> noCon; const std::vector<scalar> noVal;
    ...
    // correctNut.
    for (label c = 0; c < nC; ++c) nutF[c] = co.Cmu * k.internal[c] * k.internal[c] / eps.internal[c];
    for (std::size_t pi = 0; pi < patches.size(); ++pi) if (patches[pi].type == "wall")
