...
inline void readKOmegaSSTCoeffs(const FoamDict* ras, KOmegaSSTCoeffs& c) {
    ...
    if (!kc) return;
    c.alphaK1 = kc->scalarOr("alphaK1", c.alphaK1);          c.alphaK2 = kc->scalarOr("alphaK2", c.alphaK2);
    c.alphaOmega1 = kc->scalarOr("alphaOmega1", c.alphaOmega1); c.alphaOmega2 = kc->scalarOr("alphaOmega2", c.alphaOmega2);
    c.gamma1 = kc->scalarOr("gamma1", c.gamma1);             c.gamma2 = kc->scalarOr("gamma2", c.gamma2);
    c.beta1  = kc->scalarOr("beta1",  c.beta1);              c.beta2  = kc->scalarOr("beta2",  c.beta2);
    c.betaStar = kc->scalarOr("betaStar", c.betaStar);
    c.a1 = kc->scalarOr("a1", c.a1);  c.b1 = kc->scalarOr("b1", c.b1);  c.c1 = kc->scalarOr("c1", c.c1);
    const std::string f3 = kc->wordOr("F3", c.F3 ? "true" : "false");   // Switch: true/on/yes/1
    c.F3 = (f3 == "true" || f3 == "on" || f3 == "yes" || f3 == "1");
    c.kappa = kc->scalarOr("kappa", c.kappa);  c.E = kc->scalarOr("E", c.E);
}
