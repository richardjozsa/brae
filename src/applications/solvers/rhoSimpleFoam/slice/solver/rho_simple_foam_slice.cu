...
// readers -- none of which is re-ported), and reproduces the existing solver's output EXACTLY on a real
// tutorial. Until that holds, every later comparison against a fresh transcription would be measuring
// two unknowns at once.
//
// It is checked against compressible/rhoSimpleFoam/squareBend, and the check is equality of the written
// fields, not a tolerance: a copy that differs at all is not a copy.
//
