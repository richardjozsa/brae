...
std::vector<tensor> gaussGrad(const GeometricField<vector>& U,
    ...
    for (std::size_t pi = 0; pi < patches.size(); ++pi) {
        ...
        for (label i = 0; i < fp.size; ++i) grad[fp.faceCells[i]] += outer(Sf[fp.start + i], uv[i]);
    }
    for (label c = 0; c < nC; ++c) grad[c] = grad[c] / g.V()[c];
    return grad;
}
...
SurfaceScalarField flux(const GeometricField<vector>& U,
    ...
    for (std::size_t pi = 0; pi < patches.size(); ++pi) {
        ...
        for (label i = 0; i < fp.size; ++i) phi.boundary[pi][i] = dot(uv[i], Sf[fp.start + i]);
    }
    return phi;
}
...
SurfaceScalarField interpolate(const std::vector<scalar>& vol,
    ...
    for (label f = 0; f < nIf; ++f) sf.internal[f] = w[f] * vol[own[f]] + (1.0 - w[f]) * vol[nei[f]];
    sf.boundary.resize(patches.size());
    for (std::size_t pi = 0; pi < patches.size(); ++pi) {
        sf.boundary[pi].resize(patches[pi].size);
        for (label i = 0; i < patches[pi].size; ++i) sf.boundary[pi][i] = vol[patches[pi].faceCells[i]];
    }
    return sf;
}
...
std::vector<scalar> div(const SurfaceScalarField& phi,
    ...
    for (label f = 0; f < nIf; ++f) { d[own[f]] += phi.internal[f]; d[nei[f]] -= phi.internal[f]; }
    for (std::size_t pi = 0; pi < patches.size(); ++pi)
        for (label i = 0; i < patches[pi].size; ++i)
            d[patches[pi].faceCells[i]] += phi.boundary[pi][i];
    for (label c = 0; c < nC; ++c) d[c] /= g.V()[c];
    return d;
}
...
std::vector<vector> div(const std::vector<tensor>& tCell,
    ...
    for (label c = 0; c < nC; ++c) d[c] = d[c] / g.V()[c];
    return d;
}
