...
inline std::vector<scalar> cellWallDist(const PrimitiveMesh& m, const FvGeometry& g,
    ...
    auto update = [&](char& set, scalar& d2cur, vector& orgcur, const vector& pt, const vector& org) -> bool {
        ...
        if (!set) { d2cur = d2; orgcur = org; set = 1; return true; }
        ...
        d2cur = d2; orgcur = org; return true;
    };

    std::vector<label> changedFaces, changedCells;
    changedFaces.reserve(nFaces / 8 + 1);
    for (label f = 0; f < nFaces; ++f)
        if (isWallFace[f]) { faceOrg[f] = Cf[f]; faceD2[f] = 0.0; faceSet[f] = 1; faceQ[f] = 1; changedFaces.push_back(f); }
    ...
    while (!changedFaces.empty()) {
        ...
        for (label f : changedFaces) {
            ...
            const label o = own[f];
            if (update(cellSet[o], cellD2[o], cellOrg[o], C[o], org) && !cellQ[o]) { cellQ[o] = 1; changedCells.push_back(o); }
            if (f < nIntF) {
                const label n = nei[f];
                if (update(cellSet[n], cellD2[n], cellOrg[n], C[n], org) && !cellQ[n]) { cellQ[n] = 1; changedCells.push_back(n); }
            }
        }
        ...
        for (label c : changedCells) {
            ...
            for (label k = cfOff[c]; k < cfOff[c + 1]; ++k) {
                ...
                if (update(faceSet[f], faceD2[f], faceOrg[f], Cf[f], org) && !faceQ[f]) { faceQ[f] = 1; changedFaces.push_back(f); }
            }
        }
    }
    for (label c = 0; c < nCells; ++c) if (cellSet[c]) y[c] = std::sqrt(cellD2[c]);
    ...
    for (const FvPatch& p : patches)
        if (p.type == "wall")
            for (label i = 0; i < p.size; ++i) {
                ...
                for (label j = fo[gf]; j < fo[gf + 1]; ++j) { const label v = fv[j]; isWallVtx[v] = 1; vtxWallFaces[v].push_back(gf); }
            }
