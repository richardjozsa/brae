...
// too-small near-wall distances there, blowing up omegaWallFunction (omega_vis ∝ 1/y²).
...
// has a triangle that tilts toward p and reports a too-small distance, which on omegaWallFunction (omega_vis ∝ 1/y²)
// blows omega up ~7×. With the area-weighted centroid cf's near-wall y is byte-identical to OF's meshWave (verified
// on motorBike cells 167636/175393). (OF: meshShapes/face/face.C::centre, faceIntersection.C::nearestPointClassify.)
inline scalar pointToFaceDist(const vector& p, const std::vector<vector>& pts,
                              const std::vector<label>& fv, label v0, label v1) {
    const label n = v1 - v0;
    vector cp{0.0, 0.0, 0.0};                                         // vertex average (the centroid's seed)
    for (label j = v0; j < v1; ++j) cp += pts[fv[j]];
    cp = (1.0 / n) * cp;
    scalar sumA = 0.0; vector sumAc{0.0, 0.0, 0.0};                   // OF face::centre area-weighted centroid
    for (label j = 0; j < n; ++j) {
        const vector& a = pts[fv[v0 + j]];
        const vector& b = pts[fv[v0 + (j + 1) % n]];
        const scalar ta = mag(cross(a - cp, b - cp));                 // 2× sub-triangle area
        sumA += ta; sumAc += ta * (a + b + cp);                       // ta × (3× sub-triangle centroid)
    }
...
inline std::vector<std::vector<scalar>> nearWallDist(const PrimitiveMesh& m, const FvGeometry& g,
    ...
    for (std::size_t pi = 0; pi < patches.size(); ++pi) {
        ...
        for (label i = 0; i < wp.size; ++i) {
            ...
            for (label j = fo[f]; j < fo[f + 1]; ++j) pointFaces[fv[j]].push_back(i);
        }
        ...
        for (label i = 0; i < wp.size; ++i) {
            ...
            for (label j = fo[f]; j < fo[f + 1]; ++j)
                for (label nf : pointFaces[fv[j]]) cand.push_back(nf);
            scalar best = nwdGreat;
            for (label nf : cand) {
