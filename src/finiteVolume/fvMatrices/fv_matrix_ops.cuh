...
inline SurfaceScalarField matrixFlux(const FvScalarMatrix& M, const GeometricField<scalar>& p,
...
inline void setValues(FvScalarMatrix& M, std::vector<scalar>& psi, const PrimitiveMesh& m,
                      const std::vector<FvPatch>& patches,
                      const std::vector<label>& cells, const std::vector<scalar>& values) {
    const label nIf = m.nInternalFaces();
    const std::vector<label>& own = m.owner();
    const std::vector<label>& nei = m.neighbour();
    std::vector<std::vector<std::pair<label, bool>>> adj(m.nCells());      // cell -> (internal face, isOwner)
    for (label f = 0; f < nIf; ++f) { adj[own[f]].push_back({f, true}); adj[nei[f]].push_back({f, false}); }
    std::vector<std::vector<std::pair<label, label>>> badj(m.nCells());    // cell -> (patch, patchFace)
    for (std::size_t pi = 0; pi < patches.size(); ++pi)
        for (label i = 0; i < patches[pi].size; ++i) badj[patches[pi].faceCells[i]].push_back({(label)pi, i});
...
template <typename T>
void relaxMatrix(FvMatrix<T>& M, const GeometricField<T>& psi,
                 const PrimitiveMesh& m, const std::vector<FvPatch>& patches, scalar alpha) {
    if (alpha <= 0.0) return;
    ...
    for (label f = 0; f < nIf; ++f) { sumOff[own[f]] += std::fabs(M.upper[f]); sumOff[nei[f]] += std::fabs(M.lower[f]); }

    for (std::size_t pi = 0; pi < patches.size(); ++pi)
        for (label i = 0; i < patches[pi].size; ++i)
            M.diag[patches[pi].faceCells[i]] += cmptMagMax(M.internalCoeffs[pi][i]);
    for (label c = 0; c < nC; ++c) M.diag[c] = std::fmax(std::fabs(M.diag[c]), sumOff[c]) / alpha;
    for (std::size_t pi = 0; pi < patches.size(); ++pi)
        for (label i = 0; i < patches[pi].size; ++i)
            M.diag[patches[pi].faceCells[i]] -= cmptMin(M.internalCoeffs[pi][i]);
    for (label c = 0; c < nC; ++c) M.source[c] += (M.diag[c] - D0[c]) * psi.internal[c];
}

template <typename T>
std::vector<scalar> matrixA(const FvMatrix<T>& M, const PrimitiveMesh& m,
                            const FvGeometry& g, const std::vector<FvPatch>& patches) {
    ...
    for (label c = 0; c < nC; ++c) A[c] = D[c] / g.V()[c];
    return A;
}

inline std::vector<vector> matrixH(const FvVectorMatrix& M, const GeometricField<vector>& U,
