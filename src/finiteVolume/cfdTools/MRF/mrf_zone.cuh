...
inline void makeRelative(
    SurfaceScalarField& phi,
    const MRFZone& z,
    const FvGeometry& g,
    const std::vector<FvPatch>& fvp)
{
    mrfApplyFrameFlux(phi, z, g, fvp, +1.0);
}
...
inline void addCoriolis(
    std::vector<vector>& Usource,
    const std::vector<vector>& U,
    const std::vector<scalar>& V,
    const MRFZone& z)
{
    for (label c : z.cells)
        Usource[c] -= V[c] * cross(z.Omega, U[c]);
}
