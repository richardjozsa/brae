...
// (Omega x (Cf-origin)) & Sf, is STATIC geometry × Omega so it is precomputed once and zero outside the zone:
...
inline DeviceMRF buildDeviceMRF(const MRFZone& z, const PrimitiveMesh& m, const FvGeometry& g,
                                const std::vector<FvPatch>& fvp, const std::vector<std::string>& nonRotating = {}) {
    DeviceMRF d; d.Omega = z.Omega; d.active = true;
    const label nC = m.nCells(), nIf = m.nInternalFaces();
    std::vector<label> zc(nC, 0); for (label c : z.cells) zc[c] = 1;
    d.zoneCell.copyFrom(zc);
    std::vector<scalar> ffi(nIf, 0.0);                    // internal: both cells in zone (= z.internalFaces)
    for (label f : z.internalFaces) ffi[f] = dot(cross(z.Omega, g.Cf()[f] - z.origin), g.Sf()[f]);
    d.frameFluxInt.copyFrom(ffi);
    std::vector<scalar> ffb;                              // boundary: faceCell in zone, patch not nonRotating
    for (const FvPatch& p : fvp) {
        bool nonRot = false; for (const auto& nm : nonRotating) if (p.name == nm) { nonRot = true; break; }
