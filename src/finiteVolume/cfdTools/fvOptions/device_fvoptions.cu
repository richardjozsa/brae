...
__device__ __forceinline__ void cdComponents(scalar nu, scalar magU, scalar dx, scalar dy, scalar dz,
                                              scalar fx, scalar fy, scalar fz, scalar& cx, scalar& cy, scalar& cz) {
    cx = nu*dx + magU*0.5*fx; cy = nu*dy + magU*0.5*fy; cz = nu*dz + magU*0.5*fz;
}

__global__ void porDiagKernel(int n, const label* __restrict__ cells, scalar nu,
        scalar dx, scalar dy, scalar dz, scalar fx, scalar fy, scalar fz, const scalar* __restrict__ V,
        const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz, scalar* __restrict__ diag) {
    ...
    scalar cx, cy, cz; cdComponents(nu, magU, dx,dy,dz, fx,fy,fz, cx,cy,cz);
    diag[c] += V[c]*(cx + cy + cz);                                    // += V*isoCd  (cellZone cells unique -> no atomic)
}

__global__ void porSrcKernel(int n, const label* __restrict__ cells, int comp, scalar nu,
        scalar dx, scalar dy, scalar dz, scalar fx, scalar fy, scalar fz, const scalar* __restrict__ V,
        const scalar* __restrict__ Ux, const scalar* __restrict__ Uy, const scalar* __restrict__ Uz, scalar* __restrict__ src) {
    const int i = blockIdx.x*blockDim.x+threadIdx.x; if (i>=n) return; const int c = cells[i];
    const scalar magU = sqrt(Ux[c]*Ux[c] + Uy[c]*Uy[c] + Uz[c]*Uz[c]);
    scalar cx, cy, cz; cdComponents(nu, magU, dx,dy,dz, fx,fy,fz, cx,cy,cz);
...
void deviceFvoPorosityDiag(const DevicePorosity& por, scalar nu, const DeviceBuffer<scalar>& V,
    ...
    const int n = static_cast<int>(por.cells.size()); if (!por.active || !n) return;
...
namespace {
__global__ void limitUKernel(int n, const label* __restrict__ cells, scalar maxSqrU,
        scalar* __restrict__ Ux, scalar* __restrict__ Uy, scalar* __restrict__ Uz) {
    const int i = blockIdx.x*blockDim.x+threadIdx.x; if (i>=n) return; const int c = cells[i];
    const scalar magSqr = Ux[c]*Ux[c] + Uy[c]*Uy[c] + Uz[c]*Uz[c];
    if (magSqr > maxSqrU) { const scalar s = sqrt(maxSqrU/magSqr); Ux[c]*=s; Uy[c]*=s; Uz[c]*=s; }
}
} // namespace
...
void deviceFvoLimitVelocity(const DeviceBuffer<label>& cells, scalar maxU,
    ...
    const int n = static_cast<int>(cells.size()); if (!n) return;
...
__global__ void velDampKernel(int n, const label* __restrict__ cells, scalar UMax, scalar C,
        const scalar* __restrict__ V, const scalar* __restrict__ Ux, const scalar* __restrict__ Uy,
        const scalar* __restrict__ Uz, scalar* __restrict__ diag) {
    const int i = blockIdx.x*blockDim.x+threadIdx.x; if (i>=n) return; const int c = cells[i];
    const scalar magU = sqrt(Ux[c]*Ux[c] + Uy[c]*Uy[c] + Uz[c]*Uz[c]);
    if (magU > UMax) { const scalar s = cbrt(V[c]); diag[c] += C*s*s*(magU - UMax); }   // s*s = V^(2/3)
}
