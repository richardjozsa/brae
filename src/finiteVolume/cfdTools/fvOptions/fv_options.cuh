    ...
    const std::vector<vector>& cellCentres = {})
{
    ...
    auto ensure = [&](std::vector<scalar>& v)
    {
        if (v.empty()) v.assign(nCells, 0.0);
    };
    ...
    for (const auto& s : d.subs)
    {
        ...
        if (isRot)                                                       // rotorDiskSource (Froude BEM, scoped)
        {
            ...
            auto v3 = [](const std::vector<scalar>& a, vector df)
            {
                return a.size()>=3 ? vector{a[a.size()-3],a[a.size()-2],a[a.size()-1]} : df;
            };
