...
struct DeviceRotorDisk {
    ...
    bool   localInflow = true; vector inletVel{0,0,0};   // inletFlowType local (use U[cell]) | fixed (inletVel)
    ...
    DeviceBuffer<scalar> pAlpha, pCd, pCl; int nProf = 0;
};
...
struct RotorDiskParams {                  // filled by readFvOptions
    ...
    scalar omega = 0, tipEffect = 1, theta0 = 0; int nBlades = 0;
    bool   localInflow = true; vector inletVel{0,0,0};
