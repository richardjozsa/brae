...
inline void bladeInterp(scalar r, const std::vector<scalar>& R, const std::vector<scalar>& tw,
                        const std::vector<scalar>& ch, scalar& twist, scalar& chord) {
    const int m = (int)R.size();
    if (m == 1) { twist = tw[0]; chord = ch[0]; return; }
    int i2 = 0; while (i2 < m && R[i2] < r) ++i2;
    if (i2 == 0)      { twist = tw[0];   chord = ch[0]; }
    else if (i2 == m) { twist = tw[m-1]; chord = ch[m-1]; }
    else { const int i1 = i2-1; const scalar w = (r-R[i1])/(R[i2]-R[i1]);
           twist = tw[i1] + w*(tw[i2]-tw[i1]); chord = ch[i1] + w*(ch[i2]-ch[i1]); }
}

__device__ __forceinline__ scalar lookupCdl(scalar a, const scalar* A, const scalar* C, int n) {
    // linear interpolation of C vs A at a (A sorted ascending); clamp outside the table (OF interpolateWeights)
    if (n == 1) return C[0];
    int i2 = 0; while (i2 < n && A[i2] < a) ++i2;
    if (i2 == 0) return C[0];
    if (i2 == n) return C[n-1];
    const int i1 = i2-1; const scalar w = (a-A[i1])/(A[i2]-A[i1]);
    return C[i1] + w*(C[i2]-C[i1]);
}

__global__ void rotorForceKernel(int n, const label* __restrict__ cells, vector axis, scalar omega, scalar nBlades,
        scalar tipEffect, scalar rMax, scalar theta0, int localInflow, vector inletVel,
        const scalar* __restrict__ e2x, const scalar* __restrict__ e2y, const scalar* __restrict__ e2z,
        const scalar* __restrict__ radius, const scalar* __restrict__ area, const scalar* __restrict__ twist,
        const scalar* __restrict__ chord, const scalar* __restrict__ pA, const scalar* __restrict__ pCd,
        const scalar* __restrict__ pCl, int nProf, const scalar* __restrict__ Ux, const scalar* __restrict__ Uy,
        const scalar* __restrict__ Uz, scalar* __restrict__ fx, scalar* __restrict__ fy, scalar* __restrict__ fz) {
    const int i = blockIdx.x*blockDim.x+threadIdx.x; if (i>=n) return; const int c = cells[i];
    const scalar r = radius[i]; if (!(r > 1e-12) || !(area[i] > 1e-30)) return;
...
DeviceRotorDisk buildDeviceRotorDisk(const RotorDiskParams& p, const std::vector<vector>& cellC,
                                     const std::vector<vector>& Sf, const std::vector<label>& owner,
                                     const std::vector<label>& neighbour, label nInternalFaces) {
    DeviceRotorDisk d;
    if (!p.active || p.cells.empty()) return d;
    d.active = true; d.n = (int)p.cells.size();
    ...
    d.omega = p.omega; d.nBlades = (scalar)p.nBlades; d.tipEffect = p.tipEffect; d.theta0 = p.theta0;
    d.localInflow = p.localInflow; d.inletVel = p.inletVel;
    const std::size_t nC = cellC.size();
    std::vector<label> cellAddr(nC, -1); for (std::size_t i = 0; i < p.cells.size(); ++i) cellAddr[p.cells[i]] = (label)i;
    ...
    for (int i = 0; i < d.n; ++i) {
        ...
        scalar twist, chord; bladeInterp(r, p.bladeR, p.bladeTwist, p.bladeChord, twist, chord);
        tw[i] = twist; ch[i] = chord;
    }
    d.rMax = rMax;
    ...
    for (label f = 0; f < nInternalFaces; ++f) {
        const label o = owner[f], nb = neighbour[f]; const label oa = cellAddr[o], na = cellAddr[nb];
        const scalar mag = std::sqrt(Sf[f].x*Sf[f].x + Sf[f].y*Sf[f].y + Sf[f].z*Sf[f].z); if (mag < 1e-300) continue;
        const scalar nfDotAx = (Sf[f].x*d.axis.x + Sf[f].y*d.axis.y + Sf[f].z*d.axis.z) / mag;
        if (oa != -1 && na == -1) { if (nfDotAx > 0.8) area[oa] += mag; }
        else if (oa == -1 && na != -1) { if (-nfDotAx > 0.8) area[na] += mag; }
    }
    d.cells.copyFrom(p.cells); d.e2x.copyFrom(e2x); d.e2y.copyFrom(e2y); d.e2z.copyFrom(e2z);
    d.radius.copyFrom(rad); d.area.copyFrom(area); d.twist.copyFrom(tw); d.chord.copyFrom(ch);
    d.pAlpha.copyFrom(p.pAlpha); d.pCd.copyFrom(p.pCd); d.pCl.copyFrom(p.pCl); d.nProf = (int)p.pAlpha.size();
    return d;
}

void deviceRotorForce(const DeviceRotorDisk& rd, int nCells,
                      const DeviceBuffer<scalar>& Ux, const DeviceBuffer<scalar>& Uy, const DeviceBuffer<scalar>& Uz,
                      DeviceBuffer<scalar>& fx, DeviceBuffer<scalar>& fy, DeviceBuffer<scalar>& fz) {
    if (!rd.active || rd.n == 0) return;
    fx.resize(nCells); fy.resize(nCells); fz.resize(nCells);
