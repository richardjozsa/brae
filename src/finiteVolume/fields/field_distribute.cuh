...
template <typename T>
GeometricField<T> distributeField(const FieldData<T>& gfd, const std::vector<PatchInfo>& gPatches,
                                  const LocalMesh& lm, const std::vector<FvPatch>& lpatches,
                                  const std::vector<std::vector<scalar>>& procWeights, int /*myRank*/) {
    GeometricField<T> gf;
    gf.internal.resize(lm.mesh.nCells());
    if (gfd.internalUniform) for (auto& v : gf.internal) v = gfd.internalUniformValue;
    else for (label lc = 0; lc < lm.mesh.nCells(); ++lc) gf.internal[lc] = gfd.internalField[lm.cellProcAddr[lc]];
    ...
    for (const FvPatch& p : lpatches) {
        ...
        for (const PatchFieldData<T>& b : gfd.boundary) if (b.name == p.name) { d = &b; break; }   // literal
        if (!d) for (const PatchFieldData<T>& b : gfd.boundary)                                     // else regex ((?i) ok)
            try { if (std::regex_match(p.name, compileFoamRegex(b.name))) d = &b; } catch (const std::regex_error&) {}
        PatchFieldData<T> synth;                                                                    // constraint default
        if (!d && isConstraintPatchType(p.type)) { synth.name = p.name; synth.type = p.type; d = &synth; }
        ...
        if (d->valueUniform || d->values.empty()) {
            ...
            label gStart = -1;
            for (const PatchInfo& gp : gPatches) if (gp.name == p.name) { gStart = gp.start; break; }
            PatchFieldData<T> rd = *d; rd.values.resize(p.size);
            for (label i = 0; i < p.size; ++i) rd.values[i] = d->values[lm.faceGlobal[p.start + i] - gStart];
