...
public:
    ...
    void evaluateBoundary() {
        for (auto& b : boundary) b->initEvaluate(internal);
        Pstream::waitAll();
        for (auto& b : boundary) b->evaluate(internal);
    }
};

// Build a GeometricField from parsed file data + the mesh patches (matched by name).
template <typename T>
GeometricField<T> buildField(const FieldData<T>& fd,
    ...
    for (const FvPatch& p : patches) {
        ...
        if (!d) for (const PatchFieldData<T>& b : fd.boundary) {
            bool hit = false;
            for (const std::string& gname : p.inGroups) if (b.name == gname) { hit = true; break; }  // literal group name
            if (!hit) { try {
                const std::regex re = compileFoamRegex(b.name);
                if (std::regex_match(p.name, re)) hit = true;                                        // patch-name regex
                else for (const std::string& gname : p.inGroups) if (std::regex_match(gname, re)) { hit = true; break; }  // group-name regex
