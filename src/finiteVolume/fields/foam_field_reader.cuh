...
inline void skipToSemicolon(TokenStream& ts, int initialDepth = 0) {
    ...
    while (!(depth == 0 && ts.peek() == ";")) { const std::string s = ts.next(); if (s == "(") ++depth; else if (s == ")") --depth; }
}
...
template <typename T>
inline void readUniformOrList(TokenStream& ts, bool& uniform, T& uval, std::vector<T>& vals) {
    ...
    if (mode == "uniform") {
        ...
        ts.expect("(");
        vals.resize(n);
        for (label i = 0; i < n; ++i) vals[i] = readFoamValue<T>(ts);
        ts.expect(")");
    }
}
...
template <typename T>
inline void readValueOrInternal(TokenStream& ts, const FieldData<T>& fd, bool& uniform, T& uval, std::vector<T>& vals) {
    if (ts.peek() == "$internalField") { ts.next(); uniform = fd.internalUniform; uval = fd.internalUniformValue; vals = fd.internalField; }
...
inline std::vector<V> readBoundaryDataList(const std::string& file) {
    ...
    ts.expect("(");
    std::vector<V> vals(n);
    for (label i = 0; i < n; ++i) vals[i] = readFoamValue<V>(ts);
...
template <typename T>
inline void readTimeVaryingMapped(const std::string& fieldPath, const std::string& patchName, PatchFieldData<T>& p) {
    ...
    std::string best; double bestT = 1e300;
    for (const auto& e : fs::directory_iterator(bd)) {
        if (!e.is_directory()) continue;
        try { const double t = std::stod(e.path().filename().string()); if (t < bestT) { bestT = t; best = e.path().filename().string(); } } catch (...) {}
    }
        ...
        if (t == "internalField") {
            ...
            while (ts.peek() != "}") {
                ...
                while (ts.peek() != "}") {
                    ...
                    if (key == "type") { p.type = ts.next(); ts.expect(";"); if (p.type.rfind("atmBoundaryLayer", 0) == 0) p.hasABL = true; }
                    ...
                    else if (key == "Uref") { p.ablUref = ts.nextScalar(); ts.expect(";"); }
                    else if (key == "Zref") { p.ablZref = ts.nextScalar(); ts.expect(";"); }
                    else if (key == "z0")   { if (ts.peek() == "uniform" || ts.peek() == "constant") ts.next(); p.ablZ0 = ts.nextScalar(); ts.expect(";"); }
                    else if (key == "flowDir" || key == "zDir") { ts.expect("(");
                        const vector v{ts.nextScalar(), ts.nextScalar(), ts.nextScalar()}; ts.expect(")"); ts.expect(";");
                        if (key == "flowDir") p.ablFlowDir = v; else p.ablZDir = v; }
                    else if (key == "d" && p.hasABL)     { if (ts.peek() == "uniform" || ts.peek() == "constant") ts.next(); p.ablD = ts.nextScalar(); ts.expect(";"); }
                    else if ((key == "kappa" || key == "Cmu") && p.hasABL) { const scalar v = ts.nextScalar(); ts.expect(";"); if (key == "kappa") p.ablKappa = v; else p.ablCmu = v; }
                    // surfaceNormalFixedValue refValue / uniformNormalFixedValue uniformValue: SCALAR (U_b = refValue * n).
                        ...
                        if (m == "uniform" || m == "constant") { p.normalRefUniform = true; p.normalRefUniformValue = ts.nextScalar(); p.hasNormalRef = true; }
                        else if (m == "nonuniform") { p.normalRefUniform = false; ts.next(); const label n = ts.nextLabel();
                            ts.expect("("); p.normalRefValues.resize(n); for (label i = 0; i < n; ++i) p.normalRefValues[i] = ts.nextScalar(); ts.expect(")"); p.hasNormalRef = true; }
                        else { skipToSemicolon(ts, m == "(" ? 1 : 0); }  // table/Function1 -> ramp handles it; treat as 0
                        ts.expect(";");
                    }
                        ...
                        if (m == "constant" || m == "uniform") {
                            p.uniformValue = readFoamValue<T>(ts); p.valueUniform = true; p.hasValue = true;
                    ...
                    else if (key == "intensity")    { p.intensity    = ts.nextScalar(); ts.expect(";"); }   // turbulentIntensityKineticEnergyInlet
                    else if (key == "mixingLength") { p.mixingLength = ts.nextScalar(); ts.expect(";"); }   // turbulentMixingLength*Inlet
                        ...
                        p.hasValue = true;
                        p.inletUniform = p.valueUniform; p.inletUniformValue = p.uniformValue; p.inletValues = p.values; p.hasInletValue = true;
                        ts.expect(";");
                    }
                    else {
                ...
                if (p.type == "timeVaryingMappedFixedValue") {
                    ...
                    try { readTimeVaryingMapped(path, p.name, p); }
