...
# 5e-4. This gate has now been tightened twice, both times because a real bug was hiding under the old
# value: at 1e-2 it passed while divU came from the mass flux instead of the volumetric one (k 5.5e-3),
# and at 2e-3 it passed while the kinetic-energy source had its internal-face sign flipped (k 3.2e-4).
# Every field now lands at <= 7e-5, so 5e-4 keeps ~7x headroom for iteration-count differences between
# OF (stops on residualControl) and brae (runs its endTime) without leaving room for a physics error.
TOL=${TOL:-5e-4}
