#!/bin/bash
# Scheme gate: div(phi,h|e) limitedLinear rather than upwind. brae used to HARDCODE upwind for the
# energy equation and silently ignore fvSchemes -- a first-order downgrade that still converges. Every
# other compressible gate specifies upwind, so none of them can see it.
# Gate 3e: the same heated duct with energy sensibleInternalEnergy (OF "e") instead of sensibleEnthalpy.
#
...
SRC=${SRC:-$(cd "$(dirname "$0")/rhoLU" && pwd)}
BUILD=${BUILD:-$(cd "$(dirname "$0")/../build" && pwd)}
WORK=${WORK:-/tmp/brae_lu_vs_of}
...
# 1e-4: limitedLinear involves the NVD/TVD limiter, which is far more sensitive to gradient details than
# plain upwind (which agrees to 1.8e-7). Measured 3.6e-5, so ~3x margin -- and it still catches the
# regression this gate exists for: hardcoding upwind on the explicit div(phi,Ekp) term gives 6.6e-4.
TOL=${TOL:-1e-4}
