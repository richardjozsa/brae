...
static double hostFDES(const double g[9], double k, double om, double F1, double nut, double y, double hmax, double nu,
...
int main()
{
    ...
    double kc[nC]   = {1.0,  1.0,  0.5,  0.8};
    double omc[nC]  = {1000, 1.0,  100,  50};
    double F1c[nC]  = {1.0,  0.0,  1.0,  0.5};
    double nutc[nC] = {1e-3, 1e-5, 1e-3, 1e-4};
    double yc[nC]   = {1e-4, 1.0,  2e-4, 5e-3};
    double hmaxc[nC]= {1e-3, 1e-3, 1e-3, 2e-3};

    std::vector<scalar> hg(9*nC), hk(nC), ho(nC), hF1(nC), hnt(nC), hy(nC), hh(nC);
    for (int c = 0; c < nC; ++c) {
        hk[c]=kc[c]; ho[c]=omc[c]; hF1[c]=F1c[c]; hnt[c]=nutc[c]; hy[c]=yc[c]; hh[c]=hmaxc[c];
        for (int q = 0; q < 9; ++q) hg[q*nC + c] = g[c][q];
    }
    DeviceBuffer<scalar> k, om, F1, gradU, nut, y, hmax, FDES;
    k.copyFrom(hk); om.copyFrom(ho); F1.copyFrom(hF1); gradU.copyFrom(hg); nut.copyFrom(hnt); y.copyFrom(hy); hmax.copyFrom(hh);
    deviceKOmegaSSTIDDESfactor(nC, k, om, F1, gradU, nut, y, hmax, nu, co, FDES);
    ...
    for (int c = 0; c < nC; ++c) {
        ...
        chk(nm, d[c], hostFDES(g[c], kc[c], omc[c], F1c[c], nutc[c], yc[c], hmaxc[c], nu, co), 1e-12);
    }
    ...
    chk("all FDES > 0", (d[0]>0&&d[1]>0&&d[2]>0&&d[3]>0)?1.0:0.0, 1.0, 0.0);
