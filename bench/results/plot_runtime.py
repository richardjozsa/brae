...
x = np.arange(len(meshes)); n = len(series); w = 0.19; off = (np.arange(n) - (n-1)/2) * (w + 0.012)
...
fig.text(0.065, 0.02,
         "brae keeps pace with the 20-core CPU node and is ~5x faster than OpenFOAM's own GPU offload (AMGX / PETSc) on the same GPU.  PETSc-GPU 35.6M not run.",
