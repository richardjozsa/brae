...
| A4 | `div(phi,K)` / `div(phi,Ekp)` ignores `linearUpwind` and runs upwind. `deviceEnergyKineticSource` has no linearUpwind parameter. On `sensibleInternalEnergy`, `Ekp = 0.5\|U\|² + p/ρ` is large (p/ρ ≈ 86 kJ/kg vs e ≈ 215 kJ/kg), so first-ordering it is not cosmetic. | `device_energy.cuh:79`, `device_simple_foam.cu:1271` | medium-high | ☐ open |
| A5 | `uniformFixedValue` with a Function1/expression `uniformValue` silently reuses a stale scalar (squareBendLiq T walls → constant 350 K instead of the expression). | `foam_field_reader.cuh:334` | medium | ☐ open |
| A6 | Per-face `Prt` for `alphatWallFunction` matched by **exact patch name only**; a regex key (`"(?i).*walls"`) falls back to model Prt 1.0 instead of 0.85 → wall `alphat` and heat flux ~15% low. Same limitation in `turbulent_inlet.cuh:30,58`. | `gpuRhoSimpleFoam.cu:217` | medium | ☐ open |
...
| A10 | `overset` classified as a constraint patch type, so an overset case runs instead of being refused. | `foam_dict.cuh:41` | high if hit | ☐ open |
