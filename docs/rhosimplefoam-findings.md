...
| D5 | `validation/freestream_vs_openfoam.sh` is DEAD: it calls `/home/ghost/cudafoam/cf/validation/komega_compare.py`, a hardcoded path from the old `cf` project name, and **exits 0 anyway**. Never registered, so it has never run — but if it were registered it would pass while testing nothing. The freestream path is in fact covered by `sa_vs_openfoam` (airFoil2D uses freestreamVelocity + freestreamPressure). | ☐ delete or repair |
